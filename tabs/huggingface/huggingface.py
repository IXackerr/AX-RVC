...
def start_upload_to_huggingface(
    token,
    username,
    repo,
    model_name,
    ...
    try:
        ...
        if upload_type == "Model Only":
            # Copy model files
            os.system(
                f"cp /kaggle/working/program_ax/logs/{model_name}/{model_name}_{model_epochs}e_{model_steps}s.pth {hug_file_path}"
            )
            os.system(
                f"cp /kaggle/working/program_ax/logs/{model_name}/added*.index {hug_file_path}"
            )

            # Create zip
            if zip_preffix != "" or zip_preffix is not None:
                zip_name = (
                    f"{zip_preffix}_{model_name}_{model_epochs}e_{model_steps}s.zip"
                )
            else:
            ...
            os.system(
                f"cp -r /kaggle/working/program_ax/logs/{model_name} {hug_file_path}"
            )
            if zip_preffix != "" or zip_preffix is not None:
...
def start_download_from_huggingface(token, username, repo, zip_name):
    try:
        ...
        hf_hub_download(
            repo_id=repo_id, filename=zip_file, token=token, local_dir=hug_file_path
        )
    ...
    with gr.Tabs():
        ...
        with gr.Tab(i18n("Download Model")):
            with gr.Column():
                ...
                with gr.Row():
                    downloadlogsbut1 = gr.Button(
                        i18n("Start download"), variant="primary"
                    )
