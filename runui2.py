		...
		with gr.Tabs():
			...
			with gr.TabItem('HuggingFace 🤗'):
				...
				with gr.Accordion(label=i18n('Download Logs files for training')):
					with gr.Row():
						with gr.Column():hgf_token_gr_d=gr.Textbox(label=g);hgf_name_gr_d=gr.Textbox(label=h);hgf_repo_gr_d=gr.Textbox(label=i)
						with gr.Column():model_name_gr_d=gr.Textbox(label=j);zip_name_gr_d=gr.Textbox(label=k)
					with gr.Row():downloadlogsbut1=gr.Button('Start download',variant=A);downloadlogsinfo1=gr.Textbox(label=B,value='');downloadlogsbut1.click(start_upload_to_huggingface,[hgf_token_gr,hgf_name_gr,hgf_repo_gr,model_name_gr,zip_name_gr,what_upload_gr],[uploadinfo1])
