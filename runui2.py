...
def start_download_from_huggingface(hgf_token_gr_d,hgf_name_gr_d,hgf_repo_gr_d,zip_name_gr_d):
	hug_file_path=_A4;hug_file_name=f"{zip_name_gr_d}.zip";hug_repo_id=f"{hgf_name_gr_d}/{hgf_repo_gr_d}";destination_folder=_w;os.chdir(hug_file_path);hf_hub_download(repo_id=hug_repo_id,filename=hug_file_name,token=hgf_token_gr_d);time.sleep(2)
			...
			with gr.TabItem('HuggingFace 🤗'):
				with gr.Accordion(label=i18n('Upload Model and Logs')):
					with gr.Row():
						...
						with gr.Column():model_name_gr=gr.Textbox(label='Trained model name:');zip_name_gr=gr.Textbox(label=j);what_upload_gr=gr.Radio(label='Upload files:',choices=[_t,_A5],value=_t,interactive=_A,visible=_A)
