	...
	with gr.Blocks(title='🔊 AX-RVC',theme=gr.themes.Base(primary_hue='blue',neutral_hue='zinc'))as app:
		...
		with gr.Tabs():
			...
			with gr.TabItem('HuggingFace 🤗'):
				with gr.Row():hgf_token_gr=gr.Textbox(label='Enter HuggingFace Write Token:');hgf_name_gr=gr.Textbox(label='Enter HuggingFace Username:');hgf_repo_gr=gr.Textbox(label='Enter HuggingFace Model-Repo name:');model_name_gr=gr.Textbox(label='Trained model name:');zip_name_gr=gr.Textbox(label='Name of Zip file:');what_upload_gr=gr.Radio(label='Upload files:',choices=[g,'Model Log Folder'],value=g,interactive=_A,visible=_A);uploadbut1=gr.Button('Start upload',variant=A);uploadinfo1=gr.Textbox(label=B,value='');uploadbut1.click(preprocess_dataset,[hgf_token_gr,hgf_name_gr,hgf_repo_gr,model_name_gr,zip_name_gr,what_upload_gr],[uploadinfo1])
