...
if not os.path.isdir('lib/csvdb/'):os.makedirs('lib/csvdb');frmnt,stp=open(_O,'w'),open(_f,'w');frmnt.close();stp.close()
global DoFormant,Quefrency,Timbre
try:DoFormant,Quefrency,Timbre=CSVutil(_O,_I,_P);DoFormant=(lambda DoFormant:_A if DoFormant.lower()=='true'else _B if DoFormant.lower()=='false'else DoFormant)(DoFormant)
except(ValueError,TypeError,IndexError):DoFormant,Quefrency,Timbre=_B,1.,1.;CSVutil(_O,_Q,_P,DoFormant,Quefrency,Timbre)
load_dotenv()
config=Config()
vc=VC(config)
if config.dml==_A:
...
sup_audioext={_Z,_h,_a,'ogg','opus',_i,'mp4','aac','alac','wma','aiff','webm','ac3'}
names=[os.path.join(root,file)for(root,_,files)in os.walk(_W)for file in files if file.endswith((_V,_j))]
indexes_list=[os.path.join(root,name)for(root,_,files)in os.walk(_N,topdown=_B)for name in files if name.endswith(_X)and _Y not in name]
...
uvr5_names=[name.replace(_V,'')for name in os.listdir(weight_uvr5_root)if name.endswith(_V)or'onnx'in name]
check_for_name=lambda:sorted(names)[0]if names else''
datasets=[]
for foldername in os.listdir(os.path.join(now_dir,datasets_root)):
	if _R not in foldername:datasets.append(os.path.join(now_dir,_g,foldername))
def get_dataset():
...
def update_model_choices(select_value):
	...
	if select_value==_b:return{_J:uvr5_names,_C:_D}
	elif select_value==_k:return{_J:model_ids_list,_C:_D}
def update_dataset_list(name):
	new_datasets=[]
	for foldername in os.listdir(os.path.join(now_dir,datasets_root)):
		if _R not in foldername:new_datasets.append(os.path.join(now_dir,_g,foldername))
	return gr.Dropdown.update(choices=new_datasets)
def get_indexes():indexes_list=[os.path.join(dirpath,filename)for(dirpath,_,filenames)in os.walk('/kaggle/working/AX-RVC/logs')for filename in filenames if filename.endswith(_X)and _Y not in filename];return indexes_list if indexes_list else''
...
def uvr(model_name,inp_root,save_root_vocal,paths,save_root_ins,agg,format0,architecture):
	...
	if architecture==_b:
		try:
			...
			else:func=AudioPre if'DeEcho'not in model_name else AudioPreDeEcho;pre_fun=func(agg=int(agg),model_path=os.path.join(os.getenv(_v),model_name+_V),device=config.device,is_half=config.is_half)
...
def change_choices():names=[os.path.join(root,file)for(root,_,files)in os.walk(_W)for file in files if file.endswith((_V,_j))];indexes_list=[os.path.join(root,name)for(root,_,files)in os.walk(_N,topdown=_B)for name in files if name.endswith(_X)and _Y not in name];audio_paths=[os.path.join(root,name)for(root,_,files)in os.walk(audio_root,topdown=_B)for name in files if name.endswith(tuple(sup_audioext))and root==audio_root];return{_J:sorted(names),_C:_D},{_J:sorted(indexes_list),_C:_D},{_J:sorted(audio_paths),_C:_D}
def change_choices2():names=[os.path.join(root,file)for(root,_,files)in os.walk(_W)for file in files if file.endswith((_V,_j))];indexes_list=[os.path.join(root,name)for(root,_,files)in os.walk(_N,topdown=_B)for name in files if name.endswith(_X)and _Y not in name];return{_J:sorted(names),_C:_D},{_J:sorted(indexes_list),_C:_D}
def clean():return{_G:'',_C:_D}
def export_onnx():from lib.infer.modules.onnx.export import export_onnx as eo;eo()
sr_dict={_c:32000,_U:40000,_d:48000}
def if_done(done,p):
	while 1:
...
def formant_enabled(cbox,qfrency,tmbre,frmntapply,formantpreset,formant_refresh_button):
	if cbox:DoFormant=_A;CSVutil(_O,_Q,_P,DoFormant,qfrency,tmbre);return{_G:_A,_C:_D},{_E:_A,_C:_D},{_E:_A,_C:_D},{_E:_A,_C:_D},{_E:_A,_C:_D},{_E:_A,_C:_D}
	else:DoFormant=_B;CSVutil(_O,_Q,_P,DoFormant,qfrency,tmbre);return{_G:_B,_C:_D},{_E:_B,_C:_D},{_E:_B,_C:_D},{_E:_B,_C:_D},{_E:_B,_C:_D},{_E:_B,_C:_D},{_E:_B,_C:_D}
def formant_apply(qfrency,tmbre):Quefrency=qfrency;Timbre=tmbre;DoFormant=_A;CSVutil(_O,_Q,_P,DoFormant,qfrency,tmbre);return{_G:Quefrency,_C:_D},{_G:Timbre,_C:_D}
...
def preprocess_dataset(trainset_dir,exp_dir,sr,n_p,dataset_path):
	...
	sr=sr_dict[sr];os.makedirs(_l%(now_dir,exp_dir),exist_ok=_A);f=open(A%(now_dir,exp_dir),'w');f.close();per=3. if config.is_half else 3.7;cmd='"%s" lib/infer/modules/train/preprocess.py "%s" %s %s "%s/logs/%s" %s %.1f'%(config.python_cmd,trainset_dir,sr,n_p,now_dir,exp_dir,config.noparallel,per);logger.info(cmd);p=Popen(cmd,shell=_A);done=[_B];threading.Thread(target=if_done,args=(done,p)).start()
	while 1:
...
def extract_f0_feature(gpus,n_p,f0method,if_f0,exp_dir,version19,echl):
	A='%s/logs/%s/extract_f0_feature.log';gpus_rmvpe=gpus;gpus=gpus.split('-');os.makedirs(_l%(now_dir,exp_dir),exist_ok=_A);f=open(A%(now_dir,exp_dir),'w');f.close()
	if if_f0:
...
def change_sr2(sr2,if_f0_3,version19):path_str=''if version19==_H else _m;f0_str='f0'if if_f0_3 else'';return get_pretrained_models(path_str,f0_str,sr2)
def change_version19(sr2,if_f0_3,version19):
	path_str=''if version19==_H else _m
	if sr2==_c and version19==_H:sr2=_U
	to_return_sr2={_J:[_U,_d],_C:_D,_G:sr2}if version19==_H else{_J:[_U,_d,_c],_C:_D,_G:sr2};f0_str='f0'if if_f0_3 else'';return*get_pretrained_models(path_str,f0_str,sr2),to_return_sr2
def change_f0(if_f0_3,sr2,version19):path_str=''if version19==_H else _m;return{_E:if_f0_3,_C:_D},*get_pretrained_models(path_str,'f0',sr2)
global log_interval
...
def click_train(exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19):
	D='-pd %s';C='-pg %s';B='\\\\';A='\\';CSVutil(_f,_Q,_P,_B);exp_dir=_l%(now_dir,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);gt_wavs_dir='%s/0_gt_wavs'%exp_dir;feature_dir=_A0%exp_dir if version19==_H else _A1%exp_dir
	if if_f0_3:f0_dir='%s/2a_f0'%exp_dir;f0nsf_dir='%s/2b-f0nsf'%exp_dir;names=set([name.split(_R)[0]for name in os.listdir(gt_wavs_dir)])&set([name.split(_R)[0]for name in os.listdir(feature_dir)])&set([name.split(_R)[0]for name in os.listdir(f0_dir)])&set([name.split(_R)[0]for name in os.listdir(f0nsf_dir)])
	else:names=set([name.split(_R)[0]for name in os.listdir(gt_wavs_dir)])&set([name.split(_R)[0]for name in os.listdir(feature_dir)])
	opt=[]
	for name in names:
	...
	if version19==_H or sr2==_U:config_path='v1/%s.json'%sr2
...
def train_index(exp_dir1,version19):
	exp_dir=os.path.join(now_dir,_N,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);feature_dir=_A0%exp_dir if version19==_H else _A1%exp_dir
...
def change_info_(ckpt_path):
	...
	try:
		with open(ckpt_path.replace(os.path.basename(ckpt_path),A),_I)as f:info=eval(f.read().strip(_F).split(_F)[0].split('\t')[-1]);sr,f0=info[_y],info['if_f0'];version=_e if B in info and info[B]==_e else _H;return sr,str(f0),version
...
def cli_infer(com):
	...
	if com[14]=='False'or com[14]=='false':DoFormant=_B;Quefrency=.0;Timbre=.0;CSVutil(_O,_Q,_P,DoFormant,Quefrency,Timbre)
	else:DoFormant=_A;Quefrency=float(com[15]);Timbre=float(com[16]);CSVutil(_O,_Q,_P,DoFormant,Quefrency,Timbre)
	...
	if'Success.'in conversion_data[0]:print('Applio-RVC-Fork Infer-CLI: Inference succeeded. Writing to %s/%s...'%(_S,_T,A,output_file_name));wavfile.write('%s/%s'%(_S,_T,A,output_file_name),conversion_data[1][0],conversion_data[1][1]);print('Applio-RVC-Fork Infer-CLI: Finished! Saved output to %s/%s'%(_S,_T,A,output_file_name))
...
def print_page_details():
	...
	elif cli_current_page==_t:print('\n    arg 1) Model Path: logs/mi-test/G_168000.pth\n    arg 2) Model save name: MyModel\n    arg 3) Sample rate: 40k (32k, 40k, 48k)\n    arg 4) Has Pitch Guidance?: 1 (0 for no, 1 for yes)\n    arg 5) Model information: "My Model"\n    arg 6) Model architecture version: v2 (use either v1 or v2)\n\nExample: logs/mi-test/G_168000.pth MyModel 40k 1 "Created by Cole Mangio" v2')
def change_page(page):global cli_current_page;cli_current_page=page;return 0
def execute_command(com):
	if com=='go home':return change_page(_n)
	elif com=='go infer':return change_page(_o)
	elif com=='go pre-process':return change_page(_p)
	elif com=='go extract-feature':return change_page(_q)
	elif com=='go train':return change_page(_r)
	elif com=='go train-feature':return change_page(_s)
	elif com=='go extract-model':return change_page(_t)
	elif com[:3]=='go ':print("page '%s' does not exist!"%com[3:]);return 0
	if cli_current_page==_o:cli_infer(com)
	elif cli_current_page==_p:cli_pre_process(com)
	elif cli_current_page==_q:cli_extract_feature(com)
	elif cli_current_page==_r:cli_train(com)
	elif cli_current_page==_s:cli_train_feature(com)
	elif cli_current_page==_t:cli_extract_model(com)
...
def match_index(sid0):
	...
	sid_directory=os.path.join(_N,base_model_name);directories_to_search=[sid_directory]if os.path.exists(sid_directory)else[];directories_to_search.append(_N);matching_index_files=[]
	for directory in directories_to_search:
		for filename in os.listdir(directory):
			if filename.endswith(_X)and _Y not in filename:
...
def save_to_wav(record_button):
	...
	else:path_to_file=record_button;new_name=datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')+'.wav';target_path=os.path.join(_S,_T,os.path.basename(new_name));shutil.move(path_to_file,target_path);return target_path
def save_to_wav2_edited(dropbox):
	if dropbox is _L:0
	else:
		file_path=dropbox.name;target_path=os.path.join(_S,_T,os.path.basename(file_path))
...
def start_upload_to_huggingface(hgf_token_gr,hgf_name_gr,hgf_repo_gr,model_name_gr,zip_name_gr,what_upload_gr):
	...
	if what_upload_gr==_u:os.system(f"cp /kaggle/working/AX-RVC/logs/weights/{model_name_gr}.pth {hug_file_path}");os.system(f"cp /kaggle/working/AX-RVC/logs/{model_name_gr}/added*.index {hug_file_path}");time.sleep(5);os.walk(f"{hug_file_path}");os.system(f"zip -r {hug_file_name} {model_name_gr}.pth added*.index");os.walk('/kaggle/working/AX-RVC');api=HfApi(token=hgf_token_gr);api.upload_file(path_or_fileobj=f"{hug_file_path}/{hug_file_name}",path_in_repo=hug_file_name,repo_id=f"{hgf_name_gr}/{hgf_repo_gr}",repo_type='model');time.sleep(15);return'Succesful upload Model to Hugging Face'
mi_applio=Applio()
def GradioSetup():
		...
		with gr.Tabs():
					...
					with gr.Row():
						with gr.Column():
							...
							with gr.Row(visible=_B)as advanced_settings_batch:
								with gr.Row(label=i18n(E),open=_B):
									with gr.Column():file_index3=gr.Textbox(label=i18n(V),value='',interactive=_A);f0method1=gr.Radio(label=i18n(G),choices=[H,I,J,K,P,C,D,_K],value=_K,interactive=_A);format1=gr.Radio(label=i18n(d),choices=[_Z,_a,_h,_i],value=_Z,interactive=_A)
			...
			with gr.TabItem(i18n('Train')):
				with gr.Accordion(label=i18n('Step 1: Processing data')):
					with gr.Row():exp_dir1=gr.Textbox(label=i18n('Enter the model name:'),value=i18n('Model_Name'));sr2=gr.Radio(label=i18n('Target sample rate:'),choices=[_U,_d,_c],value=_U,interactive=_A);if_f0_3=gr.Checkbox(label=i18n('Whether the model has pitch guidance.'),value=_A,interactive=_A);version19=gr.Radio(label=i18n('Version:'),choices=[_e],value=_e,interactive=_A,visible=_A);np7=gr.Slider(minimum=1,maximum=config.n_cpu,step=1,label=i18n('Number of CPU processes:'),value=config.n_cpu,interactive=_A)
			...
			with gr.TabItem('HuggingFace 🤗'):
				with gr.Row():
					...
					with gr.Column():model_name_gr=gr.Textbox(label='Trained model name:');zip_name_gr=gr.Textbox(label='Name of Zip file:');what_upload_gr=gr.Radio(label='Upload files:',choices=[_u,'Model Log Folder'],value=_u,interactive=_A,visible=_A)
