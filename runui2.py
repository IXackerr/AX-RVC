...
def get_indexes():indexes_list=[os.path.join(dirpath,filename)for(dirpath,_,filenames)in os.walk('/kaggle/working/AX-RVC/logs')for filename in filenames if filename.endswith(_V)and _W not in filename];return indexes_list if indexes_list else''
...
def change_choices():names=[os.path.join(root,file)for(root,_,files)in os.walk('/kaggle/working/AX-RVC/logs/weights')for file in files if file.endswith((_U,_i))];indexes_list=[os.path.join(root,name)for(root,_,files)in os.walk(index_root,topdown=_B)for name in files if name.endswith(_V)and _W not in name];audio_paths=[os.path.join(root,name)for(root,_,files)in os.walk(audio_root,topdown=_B)for name in files if name.endswith(tuple(sup_audioext))and root==audio_root];return{_J:sorted(names),_C:_D},{_J:sorted(indexes_list),_C:_D},{_J:sorted(audio_paths),_C:_D}
