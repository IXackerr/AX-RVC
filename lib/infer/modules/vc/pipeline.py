...
class Pipeline(object):
    def __init__(self, tgt_sr, config):
        ...
        self.model_rmvpe = RMVPE("/kaggle/input/ax-rmf/rmvpe.pt" % os.environ["rmvpe_root"], is_half=self.is_half, device=self.device)
        self.f0_method_dict = {
            "pm": self.get_pm,
    ...
    def get_rmvpe(self, x, *args, **kwargs):
        if not hasattr(self, "model_rmvpe"):
            ...
            logger.info(
                "Loading rmvpe model,%s" % "/kaggle/input/ax-rmf/rmvpe.pt" % os.environ["rmvpe_root"]
            )
            self.model_rmvpe = RMVPE(
                "/kaggle/input/ax-rmf/rmvpe.pt" % os.environ["rmvpe_root"],
