...
_q='特征检索库文件路径,为空则使用下拉的选择结果'
_p='C:\\Users\\Desktop\\model_example.index'
	...
	with gr.Tabs():
		with gr.TabItem(i18n('模型推理')):
			...
			with gr.TabItem(i18n('单次推理')):
				with gr.Group():
					with gr.Row():
						...
						with gr.Column():vc_transform0=gr.Number(label=i18n(_o),value=0);input_audio1=gr.Dropdown(label=i18n('输入待处理音频文件路径(默认是正确格式示例)'),choices=sorted(audio_paths),value='',interactive=_A);input_audio0=gr.Textbox(label=i18n('test audio path'),placeholder=_p,interactive=_A);file_index1=gr.Textbox(label=i18n(_q),placeholder=_p,interactive=_A);file_index2=gr.Dropdown(label=i18n(_r),choices=sorted(index_paths),interactive=_A);f0method0=gr.Radio(label=i18n(_s),choices=[_R,_S,'crepe',_M]if config.dml==_B else[_R,_S,_M],value=_M,interactive=_A)
						with gr.Column():resample_sr0=gr.Slider(minimum=0,maximum=48000,label=i18n(_t),value=0,step=1,interactive=_A);rms_mix_rate0=gr.Slider(minimum=0,maximum=1,label=i18n(_u),value=.25,interactive=_A);protect0=gr.Slider(minimum=0,maximum=.5,label=i18n(_v),value=.33,step=.01,interactive=_A);filter_radius0=gr.Slider(minimum=0,maximum=7,label=i18n(_w),value=3,step=1,interactive=_A);index_rate1=gr.Slider(minimum=0,maximum=1,label=i18n(_x),value=.75,interactive=_A);f0_file=gr.File(label=i18n('F0曲线文件, 可选, 一行一个音高, 代替默认F0及升降调'),visible=_B);dropbox.upload(fn=save_to_wav2,inputs=[dropbox],outputs=[input_audio0]);record_button.change(fn=save_to_wav,inputs=[record_button],outputs=[input_audio0]);refresh_button.click(fn=change_choices,inputs=[],outputs=[sid0,file_index2,input_audio0],api_name='infer_refresh')
				with gr.Group():
					with gr.Column():
			...
			with gr.TabItem(i18n('批量推理')):
				...
				with gr.Row():
					with gr.Column():vc_transform1=gr.Number(label=i18n(_o),value=0);opt_input=gr.Textbox(label=i18n('指定输出文件夹'),value=_d);file_index3=gr.Textbox(label=i18n(_q),value='',interactive=_A);file_index4=gr.Dropdown(label=i18n(_r),choices=sorted(index_paths),interactive=_A);f0method1=gr.Radio(label=i18n(_s),choices=[_R,_S,'crepe',_M]if config.dml==_B else[_R,_S,_M],value=_M,interactive=_A);format1=gr.Radio(label=i18n(_y),choices=[_T,_U,_X,_Y],value=_T,interactive=_A);refresh_button.click(fn=lambda:change_choices()[1],inputs=[],outputs=file_index4,api_name='infer_refresh_batch')
				...
				with gr.Row():dir_input=gr.Textbox(label=i18n('输入待处理音频文件夹路径(去文件管理器地址栏拷就行了)'),placeholder='C:\\Users\\Desktop\\input_vocal_dir');inputs=gr.File(file_count=_z,label=i18n(_A0))
