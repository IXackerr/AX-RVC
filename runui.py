...
os.makedirs(os.path.join(now_dir,_o),exist_ok=_A)
os.environ['TEMP']=tmp
...
weight_root=_o
sup_audioext={_f,_p,_S,'ogg','opus',_q,'mp4','aac','alac','wma','aiff','webm','ac3'}
...
for(root,dirs,files)in os.walk(index_root,topdown=_B):
	for name in files:
		if name.endswith(_z)and _r not in name:index_paths.append(_c%(root,name))
uvr5_names=[]
		...
		for path in paths:
			...
			if'Success'in info:
				try:
					...
					if format1 in[_f,_S]:sf.write('%s/%s.%s'%(opt_root,os.path.basename(path),format1),audio_opt,tgt_sr)
					else:
			...
			infos.append('%s->%s'%(os.path.basename(path),info));yield _C.join(infos)
		yield _C.join(infos)
...
def uvr(model_name,inp_root,save_root_vocal,paths,save_root_ins,agg,format0):
	...
	try:
		...
		for path in paths:
			...
			except:infos.append('%s->%s'%(os.path.basename(inp_path),traceback.format_exc()));yield _C.join(infos)
...
def get_index_path_from_model(sid):
	sel_index_path='';name=os.path.join(_n,sid.split(_H)[0],'')
...
def get_vc(sid,to_return_protect0,to_return_protect1):
	...
	if sid==''or sid==[]:
		...
		if hubert_model is not _J:
			...
			if torch.cuda.is_available():torch.cuda.empty_cache()
		return{_Q:_B,_E:_F}
	person=_c%(weight_root,sid);print('loading %s'%person);cpt=torch.load(person,map_location=_s);tgt_sr=cpt[_I][-1];cpt[_I][-3]=cpt[_g][_A2].shape[0];if_f0=cpt.get(_R,1)
...
def change_version19(sr2,if_f0_3,version19):
	path_str=''if version19==_D else _u
	if sr2==_t and version19==_D:sr2=_X
	to_return_sr2={_h:[_X,_i],_E:_F,_W:sr2}if version19==_D else{_h:[_X,_i,_t],_E:_F,_W:sr2};f0_str=_R if if_f0_3 else'';if_pretrained_generator_exist=os.access(_Y%(path_str,f0_str,sr2),os.F_OK);if_pretrained_discriminator_exist=os.access(_Z%(path_str,f0_str,sr2),os.F_OK)
	if not if_pretrained_generator_exist:print(_Y%(path_str,f0_str,sr2),_a)
	if not if_pretrained_discriminator_exist:print(_Z%(path_str,f0_str,sr2),_a)
...
def change_f0(if_f0_3,sr2,version19):
	B='/kaggle/input/ax-rmf/pretrained%s/f0D%s.pth';A='/kaggle/input/ax-rmf/pretrained%s/f0G%s.pth';path_str=''if version19==_D else _u;if_pretrained_generator_exist=os.access(A%(path_str,sr2),os.F_OK);if_pretrained_discriminator_exist=os.access(B%(path_str,sr2),os.F_OK)
...
def click_train(exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19):
	A='\x08';exp_dir=_d%(now_dir,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);gt_wavs_dir=_A3%exp_dir;feature_dir=_v%exp_dir if version19==_D else _w%exp_dir
	...
	if gpus16:cmd=config.python_cmd+_AB%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,gpus16,total_epoch11,save_epoch10,_j%pretrained_G14 if pretrained_G14!=''else'',_k%pretrained_D15 if pretrained_D15!=''else'',1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	else:cmd=config.python_cmd+_AC%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,total_epoch11,save_epoch10,_j%pretrained_G14 if pretrained_G14!=''else A,_k%pretrained_D15 if pretrained_D15!=''else A,1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	print(cmd);p=Popen(cmd,shell=_A,cwd=now_dir);p.wait();return _AD
def train_index(exp_dir1,version19):
	exp_dir=_d%(now_dir,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);feature_dir=_v%exp_dir if version19==_D else _w%exp_dir
	if not os.path.exists(feature_dir):return'请先进行特征提取!'
	listdir_res=list(os.listdir(feature_dir))
	if len(listdir_res)==0:return'请先进行特征提取！'
	infos=[];npys=[]
	for name in sorted(listdir_res):phone=np.load(_c%(feature_dir,name));npys.append(phone)
...
def train1key(exp_dir1,sr2,if_f0_3,trainset_dir4,spk_id5,np7,f0method8,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19,gpus_rmvpe):
	...
	if gpus16:cmd=config.python_cmd+_AB%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,gpus16,total_epoch11,save_epoch10,_j%pretrained_G14 if pretrained_G14!=''else'',_k%pretrained_D15 if pretrained_D15!=''else'',1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	else:cmd=config.python_cmd+_AC%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,total_epoch11,save_epoch10,_j%pretrained_G14 if pretrained_G14!=''else'',_k%pretrained_D15 if pretrained_D15!=''else'',1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	...
	with gr.Tabs():
		with gr.TabItem(i18n('模型推理')):
			...
			with gr.Group():
				...
				with gr.Row():
					with gr.Column():vc_transform1=gr.Number(label=i18n(_AJ),value=0);opt_input=gr.Textbox(label=i18n('指定输出文件夹'),value=_y);f0method1=gr.Radio(label=i18n(_AK),choices=[_l,_x,'crepe',_m],value=_l,interactive=_A);filter_radius1=gr.Slider(minimum=0,maximum=7,label=i18n(_AL),value=3,step=1,interactive=_A)
					...
					with gr.Row():format1=gr.Radio(label=i18n('导出文件格式'),choices=[_f,_S,_p,_q],value=_S,interactive=_A);but1=gr.Button(i18n('转换'),variant=_P);vc_output3=gr.Textbox(label=i18n(_b))
		...
		with gr.TabItem(i18n('训练')):
			...
			with gr.Accordion(i18n('step2b: 使用CPU提取音高(如果模型带音高), 使用GPU提取特征(选择卡号)')):
				with gr.Row():
					...
					with gr.Column():f0method8=gr.Radio(label=i18n('选择音高提取算法:输入歌声可用pm提速,高质量语音但CPU差可用dio提速,harvest质量更好但慢'),choices=[_l,_x,'dio',_m,_e],value=_e,interactive=_A);gpus_rmvpe=gr.Textbox(label=i18n('rmvpe卡号配置：以-分隔输入使用的不同进程卡号,例如0-0-1使用在卡0上跑2个进程并在卡1上跑1个进程'),value='%s-%s'%(gpus,gpus),interactive=_A,visible=_A)
