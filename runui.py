_AP='/kaggle/input/ax-rmd/pretrained_v2'
_AO='以-分隔输入使用的卡号, 例如   0-1-2   使用卡0和卡1和卡2'
_AN='也可批量输入音频文件, 二选一, 优先读文件夹'
_AM='保护清辅音和呼吸声，防止电音撕裂等artifact，拉满0.5不开启，调低加大保护力度但可能降低索引效果'
_AL='输入源音量包络替换输出音量包络融合比例，越靠近1越使用输出包络'
_AK='后处理重采样至最终采样率，0为不进行重采样'
_AJ='自动检测index路径,下拉式选择(dropdown)'
_AI='>=3则使用对harvest音高识别的结果使用中值滤波，数值为滤波半径，使用可以削弱哑音'
_AH='选择音高提取算法,输入歌声可用pm提速,harvest低音好但巨慢无比,crepe效果好但吃GPU,rmvpe效果最好且微吃GPU'
_AG='变调(整数, 半音数量, 升八度12降八度-12)'
_AF='%s/added_IVF%s_Flat_nprobe_%s_%s_%s.index'
_AE='%s/trained_IVF%s_Flat_nprobe_%s_%s_%s.index'
_AD='IVF%s,Flat'
_AC='%s/total_fea.npy'
_AB='Trying doing kmeans %s shape to 10k centers.'
_AA='训练结束, 您可查看控制台训练日志或实验文件夹下的train.log'
_A9=' train_nsf_sim_cache_sid_load_pretrain.py -e "%s" -sr %s -f0 %s -bs %s -te %s -se %s %s %s -l %s -c %s -sw %s -v %s'
_A8=' train_nsf_sim_cache_sid_load_pretrain.py -e "%s" -sr %s -f0 %s -bs %s -g %s -te %s -se %s %s %s -l %s -c %s -sw %s -v %s'
_A7='write filelist done'
_A6='%s/filelist.txt'
_A5='%s/logs/mute/0_gt_wavs/mute%s.wav|%s/logs/mute/3_feature%s/mute.npy|%s'
_A4='%s/logs/mute/0_gt_wavs/mute%s.wav|%s/logs/mute/3_feature%s/mute.npy|%s/logs/mute/2a_f0/mute.wav.npy|%s/logs/mute/2b-f0nsf/mute.wav.npy|%s'
_A3='%s/%s.wav|%s/%s.npy|%s'
_A2='%s/%s.wav|%s/%s.npy|%s/%s.wav.npy|%s/%s.wav.npy|%s'
_A1='%s/2b-f0nsf'
_A0='%s/0_gt_wavs'
_z='emb_g.weight'
_y='clean_empty_cache'
_x='sample_rate'
_w='%s->%s'
_v='.index'
_u='opt'
...
def get_vc(sid,to_return_protect0,to_return_protect1):
	...
	if sid==''or sid==[]:
		...
		if hubert_model is not _J:
			print(_y);del net_g,n_spk,vc,hubert_model,tgt_sr;hubert_model=net_g=n_spk=vc=hubert_model=tgt_sr=_J
			if torch.cuda.is_available():torch.cuda.empty_cache()
			...
			if torch.cuda.is_available():torch.cuda.empty_cache()
		return{_Q:_B,_E:_F}
	person=sid;print('loading %s'%person);cpt=torch.load(person,map_location=_o);tgt_sr=cpt[_I][-1];cpt[_I][-3]=cpt[_f][_z].shape[0];if_f0=cpt.get(_R,1)
...
def change_choices():names=[os.path.join(root,file)for(root,_,files)in os.walk(weight_root)for file in files if file.endswith((_T,'.onnx'))];indexes_list=[os.path.join(root,name)for(root,_,files)in os.walk(index_root,topdown=_B)for name in files if name.endswith(_v)and _n not in name];audio_paths=[os.path.join(root,name)for(root,_,files)in os.walk(audio_root,topdown=_B)for name in files if name.endswith(tuple(sup_audioext))and root==audio_root];return gr.Dropdown.update(choices=sorted(names)),gr.Dropdown.update(choices=sorted(indexes_list)),gr.Dropdown.update(choices=sorted(audio_paths))
...
def change_sr2(sr2,if_f0_3,version19):
	path_str=''if version19==_D else _q;f0_str=_R if if_f0_3 else'';if_pretrained_generator_exist=os.access(_Y%(path_str,f0_str,sr2),os.F_OK);if_pretrained_discriminator_exist=os.access(_Z%(path_str,f0_str,sr2),os.F_OK)
...
def change_version19(sr2,if_f0_3,version19):
	...
	to_return_sr2={A:[_X,_g],_E:_F,_W:sr2}if version19==_D else{A:[_X,_g,_p],_E:_F,_W:sr2};f0_str=_R if if_f0_3 else'';if_pretrained_generator_exist=os.access(_Y%(path_str,f0_str,sr2),os.F_OK);if_pretrained_discriminator_exist=os.access(_Z%(path_str,f0_str,sr2),os.F_OK)
	if not if_pretrained_generator_exist:print(_Y%(path_str,f0_str,sr2),_a)
	if not if_pretrained_discriminator_exist:print(_Z%(path_str,f0_str,sr2),_a)
...
def click_train(exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19):
	A='\x08';exp_dir=_c%(now_dir,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);gt_wavs_dir=_A0%exp_dir;feature_dir=_r%exp_dir if version19==_D else _s%exp_dir
	if if_f0_3:f0_dir='%s/2a_f0'%exp_dir;f0nsf_dir=_A1%exp_dir;names=set([name.split(_H)[0]for name in os.listdir(gt_wavs_dir)])&set([name.split(_H)[0]for name in os.listdir(feature_dir)])&set([name.split(_H)[0]for name in os.listdir(f0_dir)])&set([name.split(_H)[0]for name in os.listdir(f0nsf_dir)])
	else:names=set([name.split(_H)[0]for name in os.listdir(gt_wavs_dir)])&set([name.split(_H)[0]for name in os.listdir(feature_dir)])
	opt=[]
	for name in names:
		if if_f0_3:opt.append(_A2%(gt_wavs_dir.replace(_N,_O),name,feature_dir.replace(_N,_O),name,f0_dir.replace(_N,_O),name,f0nsf_dir.replace(_N,_O),name,spk_id5))
		else:opt.append(_A3%(gt_wavs_dir.replace(_N,_O),name,feature_dir.replace(_N,_O),name,spk_id5))
	fea_dim=256 if version19==_D else 768
	if if_f0_3:
		for _ in range(2):opt.append(_A4%(now_dir,sr2,now_dir,fea_dim,now_dir,now_dir,spk_id5))
	else:
		for _ in range(2):opt.append(_A5%(now_dir,sr2,now_dir,fea_dim,spk_id5))
	shuffle(opt)
	with open(_A6%exp_dir,'w')as f:f.write(_C.join(opt))
	print(_A7);print('use gpus:',gpus16)
	if pretrained_G14=='':print('no pretrained Generator')
	if pretrained_D15=='':print('no pretrained Discriminator')
	if gpus16:cmd=config.python_cmd+_A8%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,gpus16,total_epoch11,save_epoch10,_h%pretrained_G14 if pretrained_G14!=''else'',_i%pretrained_D15 if pretrained_D15!=''else'',1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	else:cmd=config.python_cmd+_A9%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,total_epoch11,save_epoch10,_h%pretrained_G14 if pretrained_G14!=''else A,_i%pretrained_D15 if pretrained_D15!=''else A,1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	print(cmd);p=Popen(cmd,shell=_A,cwd=now_dir);p.wait();return _AA
def train_index(exp_dir1,version19):
	...
	if big_npy.shape[0]>2e5:
		infos.append(_AB%big_npy.shape[0]);yield _C.join(infos)
	...
	np.save(_AC%exp_dir,big_npy);n_ivf=min(int(16*np.sqrt(big_npy.shape[0])),big_npy.shape[0]//39);infos.append('%s,%s'%(big_npy.shape,n_ivf));yield _C.join(infos);index=faiss.index_factory(256 if version19==_D else 768,_AD%n_ivf);infos.append('training');yield _C.join(infos);index_ivf=faiss.extract_index_ivf(index);index_ivf.nprobe=1;index.train(big_npy);faiss.write_index(index,_AE%(exp_dir,n_ivf,index_ivf.nprobe,exp_dir1,version19));infos.append('adding');yield _C.join(infos);batch_size_add=8192
	for i in range(0,big_npy.shape[0],batch_size_add):index.add(big_npy[i:i+batch_size_add])
	faiss.write_index(index,_AF%(exp_dir,n_ivf,index_ivf.nprobe,exp_dir1,version19));infos.append('成功构建索引，added_IVF%s_Flat_nprobe_%s_%s_%s.index'%(n_ivf,index_ivf.nprobe,exp_dir1,version19));yield _C.join(infos)
def train1key(exp_dir1,sr2,if_f0_3,trainset_dir4,spk_id5,np7,f0method8,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19,gpus_rmvpe):
	...
	if if_f0_3:f0_dir='%s/2a_f0'%model_log_dir;f0nsf_dir=_A1%model_log_dir;names=set([name.split(_H)[0]for name in os.listdir(gt_wavs_dir)])&set([name.split(_H)[0]for name in os.listdir(feature_dir)])&set([name.split(_H)[0]for name in os.listdir(f0_dir)])&set([name.split(_H)[0]for name in os.listdir(f0nsf_dir)])
	else:names=set([name.split(_H)[0]for name in os.listdir(gt_wavs_dir)])&set([name.split(_H)[0]for name in os.listdir(feature_dir)])
	opt=[]
	for name in names:
		if if_f0_3:opt.append(_A2%(gt_wavs_dir.replace(_N,_O),name,feature_dir.replace(_N,_O),name,f0_dir.replace(_N,_O),name,f0nsf_dir.replace(_N,_O),name,spk_id5))
	...
	if if_f0_3:
		...
		for _ in range(2):opt.append(_A5%(now_dir,sr2,now_dir,fea_dim,spk_id5))
	shuffle(opt)
	with open(_A6%model_log_dir,'w')as f:f.write(_C.join(opt))
	yield get_info_str(_A7)
	if gpus16:cmd=config.python_cmd+_A8%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,gpus16,total_epoch11,save_epoch10,_h%pretrained_G14 if pretrained_G14!=''else'',_i%pretrained_D15 if pretrained_D15!=''else'',1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	else:cmd=config.python_cmd+_A9%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,total_epoch11,save_epoch10,_h%pretrained_G14 if pretrained_G14!=''else'',_i%pretrained_D15 if pretrained_D15!=''else'',1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	yield get_info_str(cmd);p=Popen(cmd,shell=_A,cwd=now_dir);p.wait();yield get_info_str(i18n(_AA));npys=[];listdir_res=list(os.listdir(feature_dir))
	for name in sorted(listdir_res):phone=np.load('%s/%s'%(feature_dir,name));npys.append(phone)
	big_npy=np.concatenate(npys,0);big_npy_idx=np.arange(big_npy.shape[0]);np.random.shuffle(big_npy_idx);big_npy=big_npy[big_npy_idx]
	if big_npy.shape[0]>2e5:
		info=_AB%big_npy.shape[0];print(info);yield get_info_str(info)
		try:big_npy=MiniBatchKMeans(n_clusters=10000,verbose=_A,batch_size=256*config.n_cpu,compute_labels=_B,init='random').fit(big_npy).cluster_centers_
		except:info=traceback.format_exc();print(info);yield get_info_str(info)
	np.save(_AC%model_log_dir,big_npy);n_ivf=min(int(16*np.sqrt(big_npy.shape[0])),big_npy.shape[0]//39);yield get_info_str('%s,%s'%(big_npy.shape,n_ivf));index=faiss.index_factory(256 if version19==_D else 768,_AD%n_ivf);yield get_info_str('training index');index_ivf=faiss.extract_index_ivf(index);index_ivf.nprobe=1;index.train(big_npy);faiss.write_index(index,_AE%(model_log_dir,n_ivf,index_ivf.nprobe,exp_dir1,version19));yield get_info_str('adding index');batch_size_add=8192
	for i in range(0,big_npy.shape[0],batch_size_add):index.add(big_npy[i:i+batch_size_add])
	faiss.write_index(index,_AF%(model_log_dir,n_ivf,index_ivf.nprobe,exp_dir1,version19));yield get_info_str('成功构建索引, added_IVF%s_Flat_nprobe_%s_%s_%s.index'%(n_ivf,index_ivf.nprobe,exp_dir1,version19));yield get_info_str(i18n('全流程结束！'))
def change_info_(ckpt_path):
	A='train.log'
	if not os.path.exists(ckpt_path.replace(os.path.basename(ckpt_path),A)):return{_E:_F},{_E:_F},{_E:_F}
	try:
		with open(ckpt_path.replace(os.path.basename(ckpt_path),A),_M)as f:info=eval(f.read().strip(_C).split(_C)[0].split('\t')[-1]);sr,f0=info[_x],info['if_f0'];version=_V if _U in info and info[_U]==_V else _D;return sr,str(f0),version
	...
	with gr.Tabs():
		with gr.TabItem(i18n('模型推理')):
			...
			with gr.TabItem(i18n('Single')):
				...
				with gr.Row():
					...
					with gr.Column():vc_transform0=gr.Number(label=i18n(_AG),value=0);f0method0=gr.Radio(label=i18n(_AH),choices=[_j,_t,'crepe',_k],value=_k,interactive=_A);filter_radius0=gr.Slider(minimum=0,maximum=7,label=i18n(_AI),value=3,step=1,interactive=_A)
					with gr.Column():
						file_index2=gr.Dropdown(label=i18n(_AJ),choices=sorted(indexes_list),interactive=_A)
					...
					with gr.Column():resample_sr0=gr.Slider(minimum=0,maximum=48000,label=i18n(_AK),value=0,step=1,interactive=_A);rms_mix_rate0=gr.Slider(minimum=0,maximum=1,label=i18n(_AL),value=.25,interactive=_A);protect0=gr.Slider(minimum=0,maximum=.5,label=i18n(_AM),value=.33,step=.01,interactive=_A)
			...
			with gr.Group():
				...
				with gr.Row():
					with gr.Column():vc_transform1=gr.Number(label=i18n(_AG),value=0);opt_input=gr.Textbox(label=i18n('指定输出文件夹'),value=_u);f0method1=gr.Radio(label=i18n(_AH),choices=[_j,_t,'crepe',_k],value=_j,interactive=_A);filter_radius1=gr.Slider(minimum=0,maximum=7,label=i18n(_AI),value=3,step=1,interactive=_A)
					with gr.Column():file_index3=gr.Textbox(label=i18n('特征检索库文件路径,为空则使用下拉的选择结果'),value='',interactive=_A);file_index4=gr.Dropdown(label=i18n(_AJ),choices=sorted(indexes_list),interactive=_A);refresh_button.click(fn=lambda:change_choices()[1],inputs=[],outputs=file_index4,api_name='infer_refresh_batch');index_rate2=gr.Slider(minimum=0,maximum=1,label=i18n('检索特征占比'),value=1,interactive=_A)
					with gr.Column():resample_sr1=gr.Slider(minimum=0,maximum=48000,label=i18n(_AK),value=0,step=1,interactive=_A);rms_mix_rate1=gr.Slider(minimum=0,maximum=1,label=i18n(_AL),value=1,interactive=_A);protect1=gr.Slider(minimum=0,maximum=.5,label=i18n(_AM),value=.33,step=.01,interactive=_A)
					with gr.Column():dir_input=gr.Textbox(label=i18n('输入待处理音频文件夹路径(去文件管理器地址栏拷就行了)'),value='E:\\codes\\py39\\test-20230416b\\todo-songs');inputs=gr.File(file_count='multiple',label=i18n(_AN))
			...
			with gr.Group():
				...
				with gr.Row():
					with gr.Column():dir_wav_input=gr.Textbox(label=i18n('输入待处理音频文件夹路径'),value='E:\\codes\\py39\\test-20230416b\\todo-songs\\todo-songs');wav_inputs=gr.File(file_count='multiple',label=i18n(_AN))
					with gr.Column():model_choose=gr.Dropdown(label=i18n('模型'),choices=uvr5_names);agg=gr.Slider(minimum=0,maximum=20,step=1,label='人声提取激进程度',value=10,interactive=_A,visible=_B);opt_vocal_root=gr.Textbox(label=i18n('指定输出主人声文件夹'),value=_u);opt_ins_root=gr.Textbox(label=i18n('指定输出非主人声文件夹'),value=_u);format0=gr.Radio(label=i18n('导出文件格式'),choices=[_e,_S,_l,_m],value=_S,interactive=_A)
		...
		with gr.TabItem(i18n('训练')):
			...
			with gr.Accordion(i18n('step2b: 使用CPU提取音高(如果模型带音高), 使用GPU提取特征(选择卡号)')):
				with gr.Row():
					with gr.Column():gpus6=gr.Textbox(label=i18n(_AO),value=gpus,interactive=_A);gpu_info9=gr.Textbox(label=i18n('显卡信息'),value=gpu_info)
