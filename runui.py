_z='要置入的模型信息'
_y='/kaggle/input/ax-rmd/pretrained_v2'
	...
	with gr.Tabs():
		with gr.TabItem(i18n('模型推理')):
			...
			with gr.TabItem(i18n('单次推理')):
				with gr.Group():
					with gr.Row():
						with gr.Column():vc_transform0=gr.Number(label=i18n(_l),value=0);input_audio0=gr.Textbox(label=i18n('输入待处理音频文件路径(默认是正确格式示例)'),placeholder='C:\\Users\\Desktop\\audio_example.wav');file_index1=gr.Textbox(label=i18n(_m),placeholder='C:\\Users\\Desktop\\model_example.index',interactive=_A);file_index2=gr.Dropdown(label=i18n(_n),choices=sorted(index_paths),interactive=_A);f0method0=gr.Radio(label=i18n(_o),choices=[_R,_S,'crepe',_M]if config.dml==_B else[_R,_S,_M],value=_M,interactive=_A)
			...
			with gr.TabItem(i18n('批量推理')):
				...
				with gr.Row():
					with gr.Column():vc_transform1=gr.Number(label=i18n(_l),value=0);opt_input=gr.Textbox(label=i18n('指定输出文件夹'),value=_b);file_index3=gr.Textbox(label=i18n(_m),value='',interactive=_A);file_index4=gr.Dropdown(label=i18n(_n),choices=sorted(index_paths),interactive=_A);f0method1=gr.Radio(label=i18n(_o),choices=[_R,_S,'crepe',_M]if config.dml==_B else[_R,_S,_M],value=_M,interactive=_A);format1=gr.Radio(label=i18n(_u),choices=[_c,_d,'mp3','m4a'],value=_c,interactive=_A);refresh_button.click(fn=lambda:change_choices()[1],inputs=[],outputs=file_index4,api_name='infer_refresh_batch')
		...
		with gr.TabItem(i18n('训练')):
			...
			with gr.Accordion(i18n('step3: 填写训练设置, 开始训练模型和索引')):
				...
				with gr.Row():file_dict={f:os.path.join(_y,f)for f in os.listdir(_y)};file_dict={k:v for(k,v)in file_dict.items()if k.endswith(_P)};file_dict_g={k:v for(k,v)in file_dict.items()if'G'in k and _Q in k};file_dict_d={k:v for(k,v)in file_dict.items()if'D'in k and _Q in k};pretrained_G14=gr.Dropdown(label=i18n('加载预训练底模G路径'),choices=list(file_dict_g.values()),value=file_dict_g['f0G32k.pth'],interactive=_A);pretrained_D15=gr.Dropdown(label=i18n('加载预训练底模D路径'),choices=list(file_dict_d.values()),value=file_dict_d['f0D32k.pth'],interactive=_A);sr2.change(change_sr2,[sr2,if_f0_3,version19],[pretrained_G14,pretrained_D15]);version19.change(change_version19,[sr2,if_f0_3,version19],[pretrained_G14,pretrained_D15,sr2]);if_f0_3.change(change_f0,[if_f0_3,sr2,version19],[f0method8,gpus_rmvpe,pretrained_G14,pretrained_D15]);gpus16=gr.Textbox(label=i18n(_x),value=gpus,interactive=_A);but3=gr.Button(i18n('训练模型'),variant=_E);but4=gr.Button(i18n('训练特征索引'),variant=_E);info3=gr.Textbox(label=i18n(_J),value='',max_lines=10);but3.click(click_train,[exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19],info3,api_name='train_start');but4.click(train_index,[exp_dir1,version19],info3)
		with gr.TabItem(i18n('ckpt处理')):
			with gr.Group():
				...
				with gr.Row():sr_=gr.Radio(label=i18n(_e),choices=[_I,_N],value=_I,interactive=_A);if_f0_=gr.Radio(label=i18n('模型是否带音高指导'),choices=[i18n(_H),i18n('否')],value=i18n(_H),interactive=_A);info__=gr.Textbox(label=i18n(_z),value='',max_lines=8,interactive=_A);name_to_save0=gr.Textbox(label=i18n('保存的模型名不带后缀'),value='',max_lines=1,interactive=_A);version_2=gr.Radio(label=i18n('模型版本型号'),choices=[_F,_L],value=_F,interactive=_A)
			...
			with gr.Group():
				...
				with gr.Row():ckpt_path2=gr.Textbox(label=i18n(_f),value='E:\\codes\\py39\\logs\\mi-test_f0_48k\\G_23333.pth',interactive=_A);save_name=gr.Textbox(label=i18n('保存名'),value='',interactive=_A);sr__=gr.Radio(label=i18n(_e),choices=[_U,_I,_N],value=_I,interactive=_A);if_f0__=gr.Radio(label=i18n('模型是否带音高指导,1是0否'),choices=['1','0'],value='1',interactive=_A);version_1=gr.Radio(label=i18n('模型版本型号'),choices=[_L],value=_L,interactive=_A);info___=gr.Textbox(label=i18n(_z),value='',max_lines=8,interactive=_A);but9=gr.Button(i18n('提取'),variant=_E);info7=gr.Textbox(label=i18n(_J),value='',max_lines=8);ckpt_path2.change(change_info_,[ckpt_path2],[sr__,if_f0__,version_1])
