...
names=[os.path.join(root,file)for(root,_,files)in os.walk(weight_root)for file in files if file.endswith((_P,'.onnx'))]
index_paths=[]
...
def lookup_indices(index_root):
	...
	for(root,dirs,files)in os.walk(index_root,topdown=_B):
		for name in files:
			if name.endswith(_g)and _h not in name:index_paths.append('%s/%s'%(root,name))
...
def click_train(exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19):
	E='-pd %s';D='-pg %s';C='\\\\';B='\\';A='.';exp_dir=_a%(now_dir,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);gt_wavs_dir='%s/0_gt_wavs'%exp_dir;feature_dir=_i%exp_dir if version19==_D else _j%exp_dir
...
def train_index(exp_dir1,version19):
	A='%s/added_IVF%s_Flat_nprobe_%s_%s_%s.index';exp_dir='logs/%s'%exp_dir1;os.makedirs(exp_dir,exist_ok=_A);feature_dir=_i%exp_dir if version19==_D else _j%exp_dir
	...
	with gr.Tabs():
		with gr.TabItem(i18n('模型推理')):
			...
			with gr.TabItem(i18n('批量推理')):
				...
				with gr.Row():
					with gr.Column():vc_transform1=gr.Number(label=i18n(_n),value=0);opt_input=gr.Textbox(label=i18n('指定输出文件夹'),value=_d);file_index3=gr.Textbox(label=i18n(_o),value='',interactive=_A);file_index4=gr.Dropdown(label=i18n(_p),choices=sorted(index_paths),interactive=_A);f0method1=gr.Radio(label=i18n(_q),choices=[_R,_S,'crepe',_M]if config.dml==_B else[_R,_S,_M],value=_M,interactive=_A);format1=gr.Radio(label=i18n(_w),choices=[_T,_U,_X,_Y],value=_T,interactive=_A);refresh_button.click(fn=lambda:change_choices()[1],inputs=[],outputs=file_index4,api_name='infer_refresh_batch')
