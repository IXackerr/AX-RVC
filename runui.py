...
_o='特征检索库文件路径,为空则使用下拉的选择结果'
_n='变调(整数, 半音数量, 升八度12降八度-12)'
_m='audios'
_l='assets'
	...
	with gr.Tabs():
		with gr.TabItem(i18n('模型推理')):
			...
			with gr.TabItem(i18n('单次推理')):
				with gr.Group():
					with gr.Row():
						...
						with gr.Column():vc_transform0=gr.Number(label=i18n(_n),value=0);input_audio0=gr.Textbox(label=i18n('输入待处理音频文件路径(默认是正确格式示例)'),placeholder='C:\\Users\\Desktop\\audio_example.wav');file_index1=gr.Textbox(label=i18n(_o),placeholder='C:\\Users\\Desktop\\model_example.index',interactive=_A);file_index2=gr.Dropdown(label=i18n(_p),choices=sorted(index_paths),interactive=_A);f0method0=gr.Radio(label=i18n(_q),choices=[_R,_S,'crepe',_M]if config.dml==_B else[_R,_S,_M],value=_M,interactive=_A)
						with gr.Column():resample_sr0=gr.Slider(minimum=0,maximum=48000,label=i18n(_r),value=0,step=1,interactive=_A);rms_mix_rate0=gr.Slider(minimum=0,maximum=1,label=i18n(_s),value=.25,interactive=_A);protect0=gr.Slider(minimum=0,maximum=.5,label=i18n(_t),value=.33,step=.01,interactive=_A);filter_radius0=gr.Slider(minimum=0,maximum=7,label=i18n(_u),value=3,step=1,interactive=_A);index_rate1=gr.Slider(minimum=0,maximum=1,label=i18n(_v),value=.75,interactive=_A);f0_file=gr.File(label=i18n('F0曲线文件, 可选, 一行一个音高, 代替默认F0及升降调'),visible=_B);dropbox.upload(fn=save_to_wav2,inputs=[dropbox],outputs=[input_audio0]);record_button.change(fn=save_to_wav,inputs=[record_button],outputs=[input_audio0]);refresh_button.click(fn=change_choices,inputs=[],outputs=[sid0,file_index2],api_name='infer_refresh')
				with gr.Group():
					with gr.Column():
			...
			with gr.TabItem(i18n('批量推理')):
				...
				with gr.Row():
					with gr.Column():vc_transform1=gr.Number(label=i18n(_n),value=0);opt_input=gr.Textbox(label=i18n('指定输出文件夹'),value=_b);file_index3=gr.Textbox(label=i18n(_o),value='',interactive=_A);file_index4=gr.Dropdown(label=i18n(_p),choices=sorted(index_paths),interactive=_A);f0method1=gr.Radio(label=i18n(_q),choices=[_R,_S,'crepe',_M]if config.dml==_B else[_R,_S,_M],value=_M,interactive=_A);format1=gr.Radio(label=i18n(_w),choices=[_c,_d,'mp3','m4a'],value=_c,interactive=_A);refresh_button.click(fn=lambda:change_choices()[1],inputs=[],outputs=file_index4,api_name='infer_refresh_batch')
