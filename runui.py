...
_AL='>=3则使用对harvest音高识别的结果使用中值滤波，数值为滤波半径，使用可以削弱哑音'
_AK='选择音高提取算法,输入歌声可用pm提速,harvest低音好但巨慢无比,crepe效果好但吃GPU,rmvpe效果最好且微吃GPU'
_AJ='变调(整数, 半音数量, 升八度12降八度-12)'
_AI='%s/added_IVF%s_Flat_nprobe_%s_%s_%s.index'
_AH='%s/trained_IVF%s_Flat_nprobe_%s_%s_%s.index'
_AG='IVF%s,Flat'
_AF='%s/total_fea.npy'
_AE='Trying doing kmeans %s shape to 10k centers.'
_AD='训练结束, 您可查看控制台训练日志或实验文件夹下的train.log'
_AC=' train_nsf_sim_cache_sid_load_pretrain.py -e "%s" -sr %s -f0 %s -bs %s -te %s -se %s %s %s -l %s -c %s -sw %s -v %s'
_AB=' train_nsf_sim_cache_sid_load_pretrain.py -e "%s" -sr %s -f0 %s -bs %s -g %s -te %s -se %s %s %s -l %s -c %s -sw %s -v %s'
_AA='write filelist done'
_A9='%s/filelist.txt'
_A8='%s/logs/mute/0_gt_wavs/mute%s.wav|%s/logs/mute/3_feature%s/mute.npy|%s'
_A7='%s/logs/mute/0_gt_wavs/mute%s.wav|%s/logs/mute/3_feature%s/mute.npy|%s/logs/mute/2a_f0/mute.wav.npy|%s/logs/mute/2b-f0nsf/mute.wav.npy|%s'
_A6='%s/%s.wav|%s/%s.npy|%s'
_A5='%s/%s.wav|%s/%s.npy|%s/%s.wav.npy|%s/%s.wav.npy|%s'
_A4='%s/2b-f0nsf'
_A3='%s/0_gt_wavs'
_A2='emb_g.weight'
_A1='clean_empty_cache'
_A0='sample_rate'
_z='%s->%s'
_y='.index'
_x='opt'
...
_Z='/kaggle/input/ax-rmf/pretrained%s/%sD%s.pth'
_Y='/kaggle/input/ax-rmf/pretrained%s/%sG%s.pth'
_X='40k'
_W='value'
_V='v2'
_U='version'
...
os.makedirs(os.path.join(now_dir,_m),exist_ok=_A)
os.makedirs(os.path.join(now_dir,_n),exist_ok=_A)
os.environ['TEMP']=tmp
...
weight_root=_n
sup_audioext={_e,_o,_S,'ogg','opus',_p,'mp4','aac','alac','wma','aiff','webm','ac3'}
...
for name in os.listdir(weight_root):
	if name.endswith(_T):names.append(name)
index_paths=[]
for(root,dirs,files)in os.walk(index_root,topdown=_B):
	for name in files:
		if name.endswith(_y)and _q not in name:index_paths.append(_f%(root,name))
uvr5_names=[]
...
def vc_multi(sid,dir_path,opt_root,paths,f0_up_key,f0_method,file_index,file_index2,index_rate,filter_radius,resample_sr,rms_mix_rate,protect,format1):
	try:
		...
		for path in paths:
			...
			infos.append(_z%(os.path.basename(path),info));yield _C.join(infos)
		yield _C.join(infos)
	...
	try:
		...
		for path in paths:
			...
			try:
				...
				if info[B][0]['channels']==2 and info[B][0][_A0]=='44100':need_reformat=0;pre_fun._path_audio_(inp_path,save_root_ins,save_root_vocal,format0);done=1
			...
			except:infos.append(_z%(os.path.basename(inp_path),traceback.format_exc()));yield _C.join(infos)
...
def get_index_path_from_model(sid):
	sel_index_path='';name=os.path.join(_m,sid.split(_H)[0],'')
...
def get_vc(sid,to_return_protect0,to_return_protect1):
	...
	if sid==''or sid==[]:
		...
		if hubert_model is not _J:
			...
			if torch.cuda.is_available():torch.cuda.empty_cache()
		return{_Q:_B,_E:_F}
	person=_f%(weight_root,sid);print('loading %s'%person);cpt=torch.load(person,map_location=_r);tgt_sr=cpt[_I][-1];cpt[_I][-3]=cpt[_g][_A2].shape[0];if_f0=cpt.get(_R,1)
...
def change_choices():names=[os.path.join(root,file)for(root,_,files)in os.walk(weight_root)for file in files if file.endswith((_T,'.onnx'))];indexes_list=[os.path.join(root,name)for(root,_,files)in os.walk(index_root,topdown=_B)for name in files if name.endswith(_y)and _q not in name];audio_paths=[os.path.join(root,name)for(root,_,files)in os.walk(audio_root,topdown=_B)for name in files if name.endswith(tuple(sup_audioext))and root==audio_root];return gr.Dropdown.update(choices=sorted(names)),gr.Dropdown.update(choices=sorted(indexes_list)),gr.Dropdown.update(choices=sorted(audio_paths))
def clean():return{_W:'',_E:_F}
sr_dict={_s:32000,_X:40000,_h:48000}
def if_done(done,p):
	while 1:
...
def change_sr2(sr2,if_f0_3,version19):
	path_str=''if version19==_D else _t;f0_str=_R if if_f0_3 else'';if_pretrained_generator_exist=os.access(_Y%(path_str,f0_str,sr2),os.F_OK);if_pretrained_discriminator_exist=os.access(_Z%(path_str,f0_str,sr2),os.F_OK)
...
def change_version19(sr2,if_f0_3,version19):
	A='choices';path_str=''if version19==_D else _t
	if sr2==_s and version19==_D:sr2=_X
	to_return_sr2={A:[_X,_h],_E:_F,_W:sr2}if version19==_D else{A:[_X,_h,_s],_E:_F,_W:sr2};f0_str=_R if if_f0_3 else'';if_pretrained_generator_exist=os.access(_Y%(path_str,f0_str,sr2),os.F_OK);if_pretrained_discriminator_exist=os.access(_Z%(path_str,f0_str,sr2),os.F_OK)
	if not if_pretrained_generator_exist:print(_Y%(path_str,f0_str,sr2),_a)
	if not if_pretrained_discriminator_exist:print(_Z%(path_str,f0_str,sr2),_a)
...
def change_f0(if_f0_3,sr2,version19):
	B='/kaggle/input/ax-rmf/pretrained%s/f0D%s.pth';A='/kaggle/input/ax-rmf/pretrained%s/f0G%s.pth';path_str=''if version19==_D else _t;if_pretrained_generator_exist=os.access(A%(path_str,sr2),os.F_OK);if_pretrained_discriminator_exist=os.access(B%(path_str,sr2),os.F_OK)
...
def click_train(exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19):
	A='\x08';exp_dir=_c%(now_dir,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);gt_wavs_dir=_A3%exp_dir;feature_dir=_u%exp_dir if version19==_D else _v%exp_dir
	if if_f0_3:f0_dir='%s/2a_f0'%exp_dir;f0nsf_dir=_A4%exp_dir;names=set([name.split(_H)[0]for name in os.listdir(gt_wavs_dir)])&set([name.split(_H)[0]for name in os.listdir(feature_dir)])&set([name.split(_H)[0]for name in os.listdir(f0_dir)])&set([name.split(_H)[0]for name in os.listdir(f0nsf_dir)])
	...
	if if_f0_3:
		...
		for _ in range(2):opt.append(_A8%(now_dir,sr2,now_dir,fea_dim,spk_id5))
	shuffle(opt)
	with open(_A9%exp_dir,'w')as f:f.write(_C.join(opt))
	print(_AA);print('use gpus:',gpus16)
	if pretrained_G14=='':print('no pretrained Generator')
	if pretrained_D15=='':print('no pretrained Discriminator')
	if gpus16:cmd=config.python_cmd+_AB%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,gpus16,total_epoch11,save_epoch10,_i%pretrained_G14 if pretrained_G14!=''else'',_j%pretrained_D15 if pretrained_D15!=''else'',1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	else:cmd=config.python_cmd+_AC%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,total_epoch11,save_epoch10,_i%pretrained_G14 if pretrained_G14!=''else A,_j%pretrained_D15 if pretrained_D15!=''else A,1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	print(cmd);p=Popen(cmd,shell=_A,cwd=now_dir);p.wait();return _AD
def train_index(exp_dir1,version19):
	exp_dir=_c%(now_dir,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);feature_dir=_u%exp_dir if version19==_D else _v%exp_dir
	if not os.path.exists(feature_dir):return'请先进行特征提取!'
	listdir_res=list(os.listdir(feature_dir))
	if len(listdir_res)==0:return'请先进行特征提取！'
	infos=[];npys=[]
	for name in sorted(listdir_res):phone=np.load(_f%(feature_dir,name));npys.append(phone)
	...
	if big_npy.shape[0]>2e5:
		infos.append(_AE%big_npy.shape[0]);yield _C.join(infos)
	...
	np.save(_AF%exp_dir,big_npy);n_ivf=min(int(16*np.sqrt(big_npy.shape[0])),big_npy.shape[0]//39);infos.append('%s,%s'%(big_npy.shape,n_ivf));yield _C.join(infos);index=faiss.index_factory(256 if version19==_D else 768,_AG%n_ivf);infos.append('training');yield _C.join(infos);index_ivf=faiss.extract_index_ivf(index);index_ivf.nprobe=1;index.train(big_npy);faiss.write_index(index,_AH%(exp_dir,n_ivf,index_ivf.nprobe,exp_dir1,version19));infos.append('adding');yield _C.join(infos);batch_size_add=8192
	for i in range(0,big_npy.shape[0],batch_size_add):index.add(big_npy[i:i+batch_size_add])
	faiss.write_index(index,_AI%(exp_dir,n_ivf,index_ivf.nprobe,exp_dir1,version19));infos.append('成功构建索引，added_IVF%s_Flat_nprobe_%s_%s_%s.index'%(n_ivf,index_ivf.nprobe,exp_dir1,version19));yield _C.join(infos)
def train1key(exp_dir1,sr2,if_f0_3,trainset_dir4,spk_id5,np7,f0method8,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19,gpus_rmvpe):
	...
	model_log_dir=_c%(now_dir,exp_dir1);preprocess_log_path='%s/preprocess.log'%model_log_dir;extract_f0_feature_log_path='%s/extract_f0_feature.log'%model_log_dir;gt_wavs_dir=_A3%model_log_dir;feature_dir=_u%model_log_dir if version19==_D else _v%model_log_dir;os.makedirs(model_log_dir,exist_ok=_A);open(preprocess_log_path,'w').close();cmd=config.python_cmd+' trainset_preprocess_pipeline_print.py "%s" %s %s "%s" '%(trainset_dir4,sr_dict[sr2],np7,model_log_dir)+str(config.noparallel);yield get_info_str(i18n('step1:正在处理数据'));yield get_info_str(cmd);p=Popen(cmd,shell=_A);p.wait()
	...
	if if_f0_3:
		...
		for _ in range(2):opt.append(_A8%(now_dir,sr2,now_dir,fea_dim,spk_id5))
	shuffle(opt)
	with open(_A9%model_log_dir,'w')as f:f.write(_C.join(opt))
	yield get_info_str(_AA)
	if gpus16:cmd=config.python_cmd+_AB%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,gpus16,total_epoch11,save_epoch10,_i%pretrained_G14 if pretrained_G14!=''else'',_j%pretrained_D15 if pretrained_D15!=''else'',1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	else:cmd=config.python_cmd+_AC%(exp_dir1,sr2,1 if if_f0_3 else 0,batch_size12,total_epoch11,save_epoch10,_i%pretrained_G14 if pretrained_G14!=''else'',_j%pretrained_D15 if pretrained_D15!=''else'',1 if if_save_latest13==i18n(_G)else 0,1 if if_cache_gpu17==i18n(_G)else 0,1 if if_save_every_weights18==i18n(_G)else 0,version19)
	yield get_info_str(cmd);p=Popen(cmd,shell=_A,cwd=now_dir);p.wait();yield get_info_str(i18n(_AD));npys=[];listdir_res=list(os.listdir(feature_dir))
	for name in sorted(listdir_res):phone=np.load(_f%(feature_dir,name));npys.append(phone)
	big_npy=np.concatenate(npys,0);big_npy_idx=np.arange(big_npy.shape[0]);np.random.shuffle(big_npy_idx);big_npy=big_npy[big_npy_idx]
	if big_npy.shape[0]>2e5:
		info=_AE%big_npy.shape[0];print(info);yield get_info_str(info)
		try:big_npy=MiniBatchKMeans(n_clusters=10000,verbose=_A,batch_size=256*config.n_cpu,compute_labels=_B,init='random').fit(big_npy).cluster_centers_
		except:info=traceback.format_exc();print(info);yield get_info_str(info)
	np.save(_AF%model_log_dir,big_npy);n_ivf=min(int(16*np.sqrt(big_npy.shape[0])),big_npy.shape[0]//39);yield get_info_str('%s,%s'%(big_npy.shape,n_ivf));index=faiss.index_factory(256 if version19==_D else 768,_AG%n_ivf);yield get_info_str('training index');index_ivf=faiss.extract_index_ivf(index);index_ivf.nprobe=1;index.train(big_npy);faiss.write_index(index,_AH%(model_log_dir,n_ivf,index_ivf.nprobe,exp_dir1,version19));yield get_info_str('adding index');batch_size_add=8192
	for i in range(0,big_npy.shape[0],batch_size_add):index.add(big_npy[i:i+batch_size_add])
	faiss.write_index(index,_AI%(model_log_dir,n_ivf,index_ivf.nprobe,exp_dir1,version19));yield get_info_str('成功构建索引, added_IVF%s_Flat_nprobe_%s_%s_%s.index'%(n_ivf,index_ivf.nprobe,exp_dir1,version19));yield get_info_str(i18n('全流程结束！'))
		...
		with gr.TabItem(i18n('模型推理')):
			...
			with gr.TabItem(i18n('Single')):
				...
				with gr.Row():
					...
					with gr.Column():vc_transform0=gr.Number(label=i18n(_AJ),value=0);f0method0=gr.Radio(label=i18n(_AK),choices=[_k,_w,'crepe',_l],value=_l,interactive=_A);filter_radius0=gr.Slider(minimum=0,maximum=7,label=i18n(_AL),value=3,step=1,interactive=_A)
					with gr.Column():
			...
			with gr.Group():
				...
				with gr.Row():
					with gr.Column():vc_transform1=gr.Number(label=i18n(_AJ),value=0);opt_input=gr.Textbox(label=i18n('指定输出文件夹'),value=_x);f0method1=gr.Radio(label=i18n(_AK),choices=[_k,_w,'crepe',_l],value=_k,interactive=_A);filter_radius1=gr.Slider(minimum=0,maximum=7,label=i18n(_AL),value=3,step=1,interactive=_A)
					with gr.Column():file_index3=gr.Textbox(label=i18n('特征检索库文件路径,为空则使用下拉的选择结果'),value='',interactive=_A);file_index4=gr.Dropdown(label=i18n(_AM),choices=sorted(index_paths),interactive=_A);refresh_button.click(fn=lambda:change_choices()[1],inputs=[],outputs=file_index4,api_name='infer_refresh_batch');index_rate2=gr.Slider(minimum=0,maximum=1,label=i18n('检索特征占比'),value=1,interactive=_A)
					...
					with gr.Row():format1=gr.Radio(label=i18n('导出文件格式'),choices=[_e,_S,_o,_p],value=_S,interactive=_A);but1=gr.Button(i18n('转换'),variant=_P);vc_output3=gr.Textbox(label=i18n(_b))
		...
		with gr.TabItem(i18n('训练')):
			...
			with gr.Accordion(i18n('step2b: 使用CPU提取音高(如果模型带音高), 使用GPU提取特征(选择卡号)')):
				with gr.Row():
					...
					with gr.Column():f0method8=gr.Radio(label=i18n('选择音高提取算法:输入歌声可用pm提速,高质量语音但CPU差可用dio提速,harvest质量更好但慢'),choices=[_k,_w,'dio',_l,_d],value=_d,interactive=_A);gpus_rmvpe=gr.Textbox(label=i18n('rmvpe卡号配置：以-分隔输入使用的不同进程卡号,例如0-0-1使用在卡0上跑2个进程并在卡1上跑1个进程'),value='%s-%s'%(gpus,gpus),interactive=_A,visible=_A)
