_A9='Output information:'
_A8='Name of Zip file:'
_A7='Enter HuggingFace Model-Repo name:'
_A6='Enter HuggingFace Username:'
_A5='Enter HuggingFace Write Token:'
_A4='/kaggle/input/ax-rmd/pretrained_v2'
_A3='以-分隔输入使用的卡号, 例如   0-1-2   使用卡0和卡1和卡2'
_A2='也可批量输入音频文件, 二选一, 优先读文件夹'
_A1='特征检索库文件路径,为空则使用下拉的选择结果'
_A0='>=3则使用对harvest音高识别的结果使用中值滤波，数值为滤波半径，使用可以削弱哑音'
_z='保护清辅音和呼吸声，防止电音撕裂等artifact，拉满0.5不开启，调低加大保护力度但可能降低索引效果'
_y='输入源音量包络替换输出音量包络融合比例，越靠近1越使用输出包络'
_x='后处理重采样至最终采样率，0为不进行重采样'
_w='选择音高提取算法,输入歌声可用pm提速,harvest低音好但巨慢无比,crepe效果好但吃GPU,rmvpe效果最好且微吃GPU'
...
os.makedirs(os.path.join(now_dir,_k),exist_ok=_A)
os.environ['TEMP']=tmp
...
datasets_root=_Y
sup_audioext={_U,_Z,_V,'ogg','opus',_a,'mp4','aac','alac','wma','aiff','webm','ac3'}
...
def extract_f0_feature(gpus,n_p,f0method,if_f0,exp_dir,version19,gpus_rmvpe):
	...
	if if_f0:
		if f0method!=_X:cmd='"%s" infer/modules/train/extract/extract_f0_print.py "%s/logs/%s" %s %s'%(config.python_cmd,now_dir,exp_dir,n_p,f0method);logger.info(_P+cmd);p=Popen(cmd,shell=_A,cwd=now_dir);done=[_B];threading.Thread(target=if_done,args=(done,p)).start()
		elif gpus_rmvpe!='-':
...
def change_sr2(sr2,if_f0_3,version19):path_str=''if version19==_D else _d;f0_str=_R if if_f0_3 else'';return get_pretrained_models(path_str,f0_str,sr2)
def change_version19(sr2,if_f0_3,version19):
	A='choices';path_str=''if version19==_D else _d
	if sr2==_W and version19==_D:sr2=_I
	to_return_sr2={A:[_I,_O],_F:_G,_b:sr2}if version19==_D else{A:[_I,_O,_W],_F:_G,_b:sr2};f0_str=_R if if_f0_3 else'';return*get_pretrained_models(path_str,f0_str,sr2),to_return_sr2
def change_f0(if_f0_3,sr2,version19):path_str=''if version19==_D else _d;return{_e:if_f0_3,_F:_G},{_e:if_f0_3,_F:_G},*get_pretrained_models(path_str,_R if if_f0_3==_A else'',sr2)
def click_train(exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19):
	D='-pd %s';C='-pg %s';B='\\\\';A='\\';exp_dir=_c%(now_dir,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);gt_wavs_dir='%s/0_gt_wavs'%exp_dir;feature_dir=_n%exp_dir if version19==_D else _o%exp_dir
	...
	logger.info(_P+cmd);p=Popen(cmd,shell=_A,cwd=now_dir);p.wait();return _p
def train_index(exp_dir1,version19):
	A='%s/added_IVF%s_Flat_nprobe_%s_%s_%s.index';exp_dir='logs/%s'%exp_dir1;os.makedirs(exp_dir,exist_ok=_A);feature_dir=_n%exp_dir if version19==_D else _o%exp_dir
...
def change_f0_method(f0method8):
	if f0method8==_X:visible=F0GPUVisible
	else:visible=_B
	return{_e:visible,_F:_G}
...
def update_dataset_list(name):
	new_datasets=[]
	for foldername in os.listdir(os.path.join(now_dir,datasets_root)):
		if _L not in foldername:new_datasets.append(os.path.join(now_dir,_Y,foldername))
...
def start_download_from_huggingface(hgf_token_gr_d,hgf_name_gr_d,hgf_repo_gr_d,zip_name_gr_d):
	hug_file_path=_s;hug_file_name=f"{zip_name_gr_d}.zip";hug_repo_id=f"{hgf_name_gr_d}/{hgf_repo_gr_d}";destination_folder='/kaggle/working/AX-RVC/logs';os.makedirs(hug_file_path);os.chdir(hug_file_path);hf_hub_download(repo_id=hug_repo_id,filename=hug_file_name,token=hgf_token_gr_d,local_dir=hug_file_path);time.sleep(2)
	...
	with gr.Tabs():
		with gr.TabItem(i18n('模型推理')):
			...
			with gr.TabItem(i18n('单次推理')):
				with gr.Row():
					...
					with gr.Column():vc_transform0=gr.Number(label=i18n(_u),value=0);input_audio0=gr.Dropdown(label=i18n('输入待处理音频文件路径(默认是正确格式示例)'),choices=sorted(audio_paths),value='',interactive=_A);file_index2=gr.Dropdown(label=i18n(_v),choices=sorted(index_paths),interactive=_A);dropbox.upload(fn=save_to_wav2,inputs=[dropbox],outputs=[input_audio0],show_progress='full');record_button.change(fn=save_to_wav,inputs=[record_button],outputs=[input_audio0]);refresh_button.click(fn=change_choices,inputs=[],outputs=[sid0,file_index2,input_audio0],api_name='infer_refresh')
				with gr.Accordion(i18n('Advanced Settings'),open=_B):
					with gr.Row():
						with gr.Column():f0method0=gr.Radio(label=i18n(_w),choices=[_S,_T,'crepe',_N]if config.dml==_B else[_S,_T,_N],value=_N,interactive=_A);resample_sr0=gr.Slider(minimum=0,maximum=48000,label=i18n(_x),value=0,step=1,interactive=_A);rms_mix_rate0=gr.Slider(minimum=0,maximum=1,label=i18n(_y),value=.25,interactive=_A)
						with gr.Column():protect0=gr.Slider(minimum=0,maximum=.5,label=i18n(_z),value=.33,step=.01,interactive=_A);filter_radius0=gr.Slider(minimum=0,maximum=7,label=i18n(_A0),value=3,step=1,interactive=_A);index_rate1=gr.Slider(minimum=0,maximum=1,label=i18n('检索特征占比'),value=.75,interactive=_A);f0_file=gr.File(label=i18n('F0曲线文件, 可选, 一行一个音高, 代替默认F0及升降调'),visible=_B);file_index1=gr.Textbox(label=i18n(_A1),placeholder='C:\\Users\\Desktop\\model_example.index',interactive=_A,visible=_B)
			...
			with gr.TabItem(i18n('批量推理')):
				...
				with gr.Row():
					with gr.Column():vc_transform1=gr.Number(label=i18n(_u),value=0);opt_input=gr.Textbox(label=i18n('指定输出文件夹'),value=_h);file_index3=gr.Textbox(label=i18n(_A1),value='',interactive=_A);file_index4=gr.Dropdown(label=i18n(_v),choices=sorted(index_paths),interactive=_A);f0method1=gr.Radio(label=i18n(_w),choices=[_S,_T,'crepe',_N]if config.dml==_B else[_S,_T,_N],value=_N,interactive=_A);format1=gr.Radio(label=i18n('导出文件格式'),choices=[_U,_V,_Z,_a],value=_U,interactive=_A);refresh_button.click(fn=lambda:change_choices()[1],inputs=[],outputs=file_index4,api_name='infer_refresh_batch')
					with gr.Column():resample_sr1=gr.Slider(minimum=0,maximum=48000,label=i18n(_x),value=0,step=1,interactive=_A);rms_mix_rate1=gr.Slider(minimum=0,maximum=1,label=i18n(_y),value=1,interactive=_A);protect1=gr.Slider(minimum=0,maximum=.5,label=i18n(_z),value=.33,step=.01,interactive=_A);filter_radius1=gr.Slider(minimum=0,maximum=7,label=i18n(_A0),value=3,step=1,interactive=_A);index_rate2=gr.Slider(minimum=0,maximum=1,label=i18n('检索特征占比'),value=1,interactive=_A)
				with gr.Row():dir_input=gr.Textbox(label=i18n('输入待处理音频文件夹路径(去文件管理器地址栏拷就行了)'),placeholder='C:\\Users\\Desktop\\input_vocal_dir');inputs=gr.File(file_count='multiple',label=i18n(_A2))
		...
		with gr.TabItem(i18n('伴奏人声分离&去混响&去回声')):
			with gr.Group():
				...
				with gr.Row():
					with gr.Column():dir_wav_input=gr.Textbox(label=i18n('输入待处理音频文件夹路径'),placeholder='C:\\Users\\Desktop\\todo-songs');wav_inputs=gr.File(file_count='multiple',label=i18n(_A2))
					with gr.Column():model_choose=gr.Dropdown(label=i18n('模型'),choices=uvr5_names);agg=gr.Slider(minimum=0,maximum=20,step=1,label='人声提取激进程度',value=10,interactive=_A,visible=_B);opt_vocal_root=gr.Textbox(label=i18n('指定输出主人声文件夹'),value=_h);opt_ins_root=gr.Textbox(label=i18n('指定输出非主人声文件夹'),value=_h);format0=gr.Radio(label=i18n('导出文件格式'),choices=[_U,_V,_Z,_a],value=_V,interactive=_A)
		...
		with gr.TabItem(i18n('训练')):
			with gr.Accordion(i18n('step1: 填写实验配置. 实验数据放在logs下, 每个实验一个文件夹, 需手工输入实验名路径, 内含实验配置, 日志, 训练得到的模型文件. ')):
				with gr.Row():exp_dir1=gr.Textbox(label=i18n('输入实验名'),value='mi-test');sr2=gr.Radio(label=i18n(_i),choices=[_I,_O],value=_I,interactive=_A);if_f0_3=gr.Radio(label=i18n('模型是否带音高指导(唱歌一定要, 语音可以不要)'),choices=[_A,_B],value=_A,interactive=_A);version19=gr.Radio(label=i18n('版本'),choices=[_M],value=_M,interactive=_A,visible=_A);np7=gr.Slider(minimum=0,maximum=config.n_cpu,step=1,label=i18n('提取音高和处理数据使用的CPU进程数'),value=int(np.ceil(config.n_cpu/1.5)),interactive=_A)
			...
			with gr.Accordion(i18n('step2b: 使用CPU提取音高(如果模型带音高), 使用GPU提取特征(选择卡号)')):
				with gr.Row():
					with gr.Column():gpus6=gr.Textbox(label=i18n(_A3),value=gpus,interactive=_A,visible=F0GPUVisible);gpu_info9=gr.Textbox(label=i18n('显卡信息'),value=gpu_info,visible=F0GPUVisible)
					with gr.Column():f0method8=gr.Radio(label=i18n('选择音高提取算法:输入歌声可用pm提速,高质量语音但CPU差可用dio提速,harvest质量更好但慢,rmvpe效果最好且微吃CPU/GPU'),choices=[_S,_T,'dio',_N,_X],value=_X,interactive=_A);gpus_rmvpe=gr.Textbox(label=i18n('rmvpe卡号配置：以-分隔输入使用的不同进程卡号,例如0-0-1使用在卡0上跑2个进程并在卡1上跑1个进程'),value='%s-%s'%(gpus,gpus),interactive=_A,visible=F0GPUVisible)
			...
			with gr.Accordion(i18n('step3: 填写训练设置, 开始训练模型和索引')):
				...
				with gr.Row():file_dict={f:os.path.join(_A4,f)for f in os.listdir(_A4)};file_dict={k:v for(k,v)in file_dict.items()if k.endswith(_Q)};file_dict_g={k:v for(k,v)in file_dict.items()if'G'in k and _R in k};file_dict_d={k:v for(k,v)in file_dict.items()if'D'in k and _R in k};pretrained_G14=gr.Dropdown(label=i18n('加载预训练底模G路径'),choices=list(file_dict_g.values()),value=file_dict_g['f0G32k.pth'],interactive=_A);pretrained_D15=gr.Dropdown(label=i18n('加载预训练底模D路径'),choices=list(file_dict_d.values()),value=file_dict_d['f0D32k.pth'],interactive=_A);sr2.change(change_sr2,[sr2,if_f0_3,version19],[pretrained_G14,pretrained_D15]);version19.change(change_version19,[sr2,if_f0_3,version19],[pretrained_G14,pretrained_D15,sr2]);if_f0_3.change(change_f0,[if_f0_3,sr2,version19],[f0method8,gpus_rmvpe,pretrained_G14,pretrained_D15]);gpus16=gr.Textbox(label=i18n(_A3),value=gpus,interactive=_A);but3=gr.Button(i18n('训练模型'),variant=_C);but4=gr.Button(i18n('训练特征索引'),variant=_C);info3=gr.Textbox(label=i18n(_K),value='',max_lines=10);but3.click(click_train,[exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19],info3,api_name='train_start');but4.click(train_index,[exp_dir1,version19],info3)
		with gr.TabItem('HuggingFace 🤗'):
			with gr.Accordion(label=i18n('Upload Model and Logs')):
				with gr.Row():
					with gr.Column():hgf_token_gr=gr.Textbox(label=_A5);hgf_name_gr=gr.Textbox(label=_A6);hgf_repo_gr=gr.Textbox(label=_A7)
					with gr.Column():model_name_gr=gr.Textbox(label='Trained model name:');zip_name_gr=gr.Textbox(label=_A8);what_upload_gr=gr.Radio(label='Upload files:',choices=[_f,_t],value=_f,interactive=_A,visible=_A)
				with gr.Row():uploadbut1=gr.Button('Start upload',variant=_C);uploadinfo1=gr.Textbox(label=_A9,value='');uploadbut1.click(start_upload_to_huggingface,[hgf_token_gr,hgf_name_gr,hgf_repo_gr,model_name_gr,zip_name_gr,what_upload_gr],[uploadinfo1])
			with gr.Accordion(label=i18n('Download Logs files for training')):
				with gr.Row():
					with gr.Column():hgf_token_gr_d=gr.Textbox(label=_A5);hgf_name_gr_d=gr.Textbox(label=_A6)
					with gr.Column():hgf_repo_gr_d=gr.Textbox(label=_A7);zip_name_gr_d=gr.Textbox(label=_A8)
				with gr.Row():downloadlogsbut1=gr.Button('Start download',variant=_C);downloadlogsinfo1=gr.Textbox(label=_A9,value='');downloadlogsbut1.click(start_download_from_huggingface,[hgf_token_gr_d,hgf_name_gr_d,hgf_repo_gr_d,zip_name_gr_d],[downloadlogsinfo1])
		...
		with gr.TabItem(i18n('ckpt处理')):
			with gr.Group():
				...
				with gr.Row():sr_=gr.Radio(label=i18n(_i),choices=[_I,_O],value=_I,interactive=_A);if_f0_=gr.Radio(label=i18n('模型是否带音高指导'),choices=[i18n(_H),i18n('否')],value=i18n(_H),interactive=_A);info__=gr.Textbox(label=i18n('要置入的模型信息'),value='',max_lines=8,interactive=_A);name_to_save0=gr.Textbox(label=i18n('保存的模型名不带后缀'),value='',max_lines=1,interactive=_A);version_2=gr.Radio(label=i18n('模型版本型号'),choices=[_D,_M],value=_D,interactive=_A)
			...
			with gr.Group():
				...
				with gr.Row():ckpt_path0=gr.Textbox(label=i18n(_j),value='',interactive=_A);info_=gr.Textbox(label=i18n('要改的模型信息'),value='',max_lines=8,interactive=_A);name_to_save1=gr.Textbox(label=i18n('保存的文件名, 默认空为和源文件同名'),value='',max_lines=8,interactive=_A)
			...
			with gr.Group():
				...
				with gr.Row():ckpt_path1=gr.Textbox(label=i18n(_j),value='',interactive=_A);but8=gr.Button(i18n('查看'),variant=_C);info6=gr.Textbox(label=i18n(_K),value='',max_lines=8)
				but8.click(show_info,[ckpt_path1],info6,api_name='ckpt_show')
			with gr.Group():
				gr.Markdown(value=i18n('模型提取(输入logs文件夹下大文件模型路径),适用于训一半不想训了模型没有自动提取保存小文件模型,或者想测试中间模型的情况'))
				with gr.Row():ckpt_path2=gr.Textbox(label=i18n(_j),value='E:\\codes\\py39\\logs\\mi-test_f0_48k\\G_23333.pth',interactive=_A);save_name=gr.Textbox(label=i18n('保存名'),value='',interactive=_A);sr__=gr.Radio(label=i18n(_i),choices=[_W,_I,_O],value=_I,interactive=_A);if_f0__=gr.Radio(label=i18n('模型是否带音高指导,1是0否'),choices=['1','0'],value='1',interactive=_A);version_1=gr.Radio(label=i18n('模型版本型号'),choices=[_M],value=_M,interactive=_A);info___=gr.Textbox(label=i18n('要置入的模型信息'),value='',max_lines=8,interactive=_A);but9=gr.Button(i18n('提取'),variant=_C);info7=gr.Textbox(label=i18n(_K),value='',max_lines=8);ckpt_path2.change(change_info_,[ckpt_path2],[sr__,if_f0__,version_1])
