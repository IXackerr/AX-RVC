...
for(root,dirs,files)in os.walk(index_root,topdown=_B):
	for name in files:
		if name.endswith(_x)and _n not in name:index_paths.append(_b%(root,name))
uvr5_names=[]
		...
		for path in paths:
			...
			if'Success'in info:
				try:
					...
					if format1 in[_o,_c]:sf.write('%s/%s.%s'%(opt_root,os.path.basename(path),format1),audio_opt,tgt_sr)
					else:
...
def get_vc(sid,to_return_protect0,to_return_protect1):
	...
	if sid==''or sid==[]:
		...
		if hubert_model is not _O:
			...
			if torch.cuda.is_available():torch.cuda.empty_cache()
		return{_Q:_B,_E:_F}
	person=_b%(weight_root,sid);print('loading %s'%person);cpt=torch.load(person,map_location=_p);tgt_sr=cpt[_I][-1];cpt[_I][-3]=cpt[_f][_A1].shape[0];if_f0=cpt.get(_R,1)
...
def change_choices():
	...
	for(root,dirs,files)in os.walk(index_root,topdown=_B):
		for name in files:
			if name.endswith(_x)and _n not in name:index_paths.append(_b%(root,name))
	return{_g:sorted(names),_E:_F},{_g:sorted(index_paths),_E:_F}
def clean():return{_V:'',_E:_F}
sr_dict={_q:32000,_W:40000,_h:48000}
def if_done(done,p):
	while 1:
...
def change_sr2(sr2,if_f0_3,version19):
	path_str=''if version19==_D else _r;f0_str=_R if if_f0_3 else'';if_pretrained_generator_exist=os.access(_X%(path_str,f0_str,sr2),os.F_OK);if_pretrained_discriminator_exist=os.access(_Y%(path_str,f0_str,sr2),os.F_OK)
...
def change_version19(sr2,if_f0_3,version19):
	path_str=''if version19==_D else _r
	if sr2==_q and version19==_D:sr2=_W
	to_return_sr2={_g:[_W,_h],_E:_F,_V:sr2}if version19==_D else{_g:[_W,_h,_q],_E:_F,_V:sr2};f0_str=_R if if_f0_3 else'';if_pretrained_generator_exist=os.access(_X%(path_str,f0_str,sr2),os.F_OK);if_pretrained_discriminator_exist=os.access(_Y%(path_str,f0_str,sr2),os.F_OK)
	if not if_pretrained_generator_exist:print(_X%(path_str,f0_str,sr2),_Z)
	if not if_pretrained_discriminator_exist:print(_Y%(path_str,f0_str,sr2),_Z)
...
def change_f0(if_f0_3,sr2,version19):
	B='/kaggle/input/ax-rmf/pretrained%s/f0D%s.pth';A='/kaggle/input/ax-rmf/pretrained%s/f0G%s.pth';path_str=''if version19==_D else _r;if_pretrained_generator_exist=os.access(A%(path_str,sr2),os.F_OK);if_pretrained_discriminator_exist=os.access(B%(path_str,sr2),os.F_OK)
...
def click_train(exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19):
	A='\x08';exp_dir=_d%(now_dir,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);gt_wavs_dir=_A2%exp_dir;feature_dir=_s%exp_dir if version19==_D else _t%exp_dir
...
def train_index(exp_dir1,version19):
	exp_dir=_d%(now_dir,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);feature_dir=_s%exp_dir if version19==_D else _t%exp_dir
	...
	with gr.Tabs():
		with gr.TabItem(i18n('模型推理')):
			...
			with gr.Group():
				...
				with gr.Row():
					with gr.Column():vc_transform0=gr.Number(label=i18n(_AI),value=0);input_audio0=gr.Textbox(label=i18n('输入待处理音频文件路径(默认是正确格式示例)'),value='E:\\codes\\py39\\test-20230416b\\todo-songs\\冬之花clip1.wav');f0method0=gr.Radio(label=i18n(_AJ),choices=[_k,_u,'crepe',_l],value=_l,interactive=_A);filter_radius0=gr.Slider(minimum=0,maximum=7,label=i18n(_AK),value=3,step=1,interactive=_A)
			...
			with gr.Group():
				...
				with gr.Row():
					with gr.Column():vc_transform1=gr.Number(label=i18n(_AI),value=0);opt_input=gr.Textbox(label=i18n('指定输出文件夹'),value=_v);f0method1=gr.Radio(label=i18n(_AJ),choices=[_k,_u,'crepe',_l],value=_k,interactive=_A);filter_radius1=gr.Slider(minimum=0,maximum=7,label=i18n(_AK),value=3,step=1,interactive=_A)
					...
					with gr.Row():format1=gr.Radio(label=i18n('导出文件格式'),choices=[_o,_c,'mp3','m4a'],value=_c,interactive=_A);but1=gr.Button(i18n('转换'),variant=_P);vc_output3=gr.Textbox(label=i18n(_a))
		...
		with gr.TabItem(i18n('训练')):
			with gr.Accordion(i18n('step1: 填写实验配置. 实验数据放在logs下, 每个实验一个文件夹, 需手工输入实验名路径, 内含实验配置, 日志, 训练得到的模型文件. ')):
				with gr.Row():exp_dir1=gr.Textbox(label=i18n('输入实验名'),value='mi-test');sr2=gr.Radio(label=i18n('目标采样率'),choices=[_W,_h],value=_W,interactive=_A);if_f0_3=gr.Radio(label=i18n('模型是否带音高指导(唱歌一定要, 语音可以不要)'),choices=[_A,_B],value=_A,interactive=_A);version19=gr.Radio(label=i18n('版本'),choices=[_U],value=_U,interactive=_A,visible=_A);np7=gr.Slider(minimum=0,maximum=config.n_cpu,step=1,label=i18n('提取音高和处理数据使用的CPU进程数'),value=int(np.ceil(config.n_cpu/1.5)),interactive=_A)
			...
			with gr.Accordion(i18n('step2b: 使用CPU提取音高(如果模型带音高), 使用GPU提取特征(选择卡号)')):
				with gr.Row():
					...
					with gr.Column():f0method8=gr.Radio(label=i18n('选择音高提取算法:输入歌声可用pm提速,高质量语音但CPU差可用dio提速,harvest质量更好但慢'),choices=[_k,_u,'dio',_l,_e],value=_e,interactive=_A);gpus_rmvpe=gr.Textbox(label=i18n('rmvpe卡号配置：以-分隔输入使用的不同进程卡号,例如0-0-1使用在卡0上跑2个进程并在卡1上跑1个进程'),value='%s-%s'%(gpus,gpus),interactive=_A,visible=_A)
