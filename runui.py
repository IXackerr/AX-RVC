_A2='/kaggle/input/ax-rmd/pretrained_v2'
_A1='以-分隔输入使用的卡号, 例如   0-1-2   使用卡0和卡1和卡2'
_A0='也可批量输入音频文件, 二选一, 优先读文件夹'
_z='导出文件格式'
_y='特征检索库文件路径,为空则使用下拉的选择结果'
_x='检索特征占比'
_w='>=3则使用对harvest音高识别的结果使用中值滤波，数值为滤波半径，使用可以削弱哑音'
_v='保护清辅音和呼吸声，防止电音撕裂等artifact，拉满0.5不开启，调低加大保护力度但可能降低索引效果'
_u='输入源音量包络替换输出音量包络融合比例，越靠近1越使用输出包络'
_t='后处理重采样至最终采样率，0为不进行重采样'
_s='选择音高提取算法,输入歌声可用pm提速,harvest低音好但巨慢无比,crepe效果好但吃GPU,rmvpe效果最好且微吃GPU'
_r='自动检测index路径,下拉式选择(dropdown)'
_q='变调(整数, 半音数量, 升八度12降八度-12)'
_p='audios'
_o='assets'
_n='训练结束, 您可查看控制台训练日志或实验文件夹下的train.log'
_m='%s/3_feature768'
_l='%s/3_feature256'
_k='trained'
_j='.index'
...
os.makedirs(os.path.join(now_dir,_i),exist_ok=_A)
os.environ['TEMP']=tmp
...
def click_train(exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19):
	D='-pd %s';C='-pg %s';B='\\\\';A='\\';exp_dir=_c%(now_dir,exp_dir1);os.makedirs(exp_dir,exist_ok=_A);gt_wavs_dir='%s/0_gt_wavs'%exp_dir;feature_dir=_l%exp_dir if version19==_D else _m%exp_dir
	...
	logger.info(_P+cmd);p=Popen(cmd,shell=_A,cwd=now_dir);p.wait();return _n
def train_index(exp_dir1,version19):
	A='%s/added_IVF%s_Flat_nprobe_%s_%s_%s.index';exp_dir='logs/%s'%exp_dir1;os.makedirs(exp_dir,exist_ok=_A);feature_dir=_l%exp_dir if version19==_D else _m%exp_dir
...
def save_to_wav(record_button):
	...
	else:path_to_file=record_button;new_name=datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')+'.wav';target_path=os.path.join(_o,_p,os.path.basename(new_name));shutil.move(path_to_file,target_path);return target_path
def save_to_wav2(dropbox):
	file_path=dropbox.name;target_path=os.path.join(_o,_p,os.path.basename(file_path))
	...
	with gr.Tabs():
		with gr.TabItem(i18n('模型推理')):
			...
			with gr.TabItem(i18n('单次推理')):
				with gr.Row():
					...
					with gr.Column():vc_transform0=gr.Number(label=i18n(_q),value=0);input_audio0=gr.Dropdown(label=i18n('输入待处理音频文件路径(默认是正确格式示例)'),choices=sorted(audio_paths),value='',interactive=_A);file_index2=gr.Dropdown(label=i18n(_r),choices=sorted(index_paths),interactive=_A);dropbox.upload(fn=save_to_wav2,inputs=[dropbox],outputs=[input_audio0],show_progress='full');record_button.change(fn=save_to_wav,inputs=[record_button],outputs=[input_audio0]);refresh_button.click(fn=change_choices,inputs=[],outputs=[sid0,file_index2,input_audio0],api_name='infer_refresh')
				with gr.Accordion(i18n('Advanced Settings'),open=_B):
					with gr.Row():
						with gr.Column():f0method0=gr.Radio(label=i18n(_s),choices=[_S,_T,'crepe',_N]if config.dml==_B else[_S,_T,_N],value=_N,interactive=_A);resample_sr0=gr.Slider(minimum=0,maximum=48000,label=i18n(_t),value=0,step=1,interactive=_A);rms_mix_rate0=gr.Slider(minimum=0,maximum=1,label=i18n(_u),value=.25,interactive=_A)
						with gr.Column():protect0=gr.Slider(minimum=0,maximum=.5,label=i18n(_v),value=.33,step=.01,interactive=_A);filter_radius0=gr.Slider(minimum=0,maximum=7,label=i18n(_w),value=3,step=1,interactive=_A);index_rate1=gr.Slider(minimum=0,maximum=1,label=i18n(_x),value=.75,interactive=_A);f0_file=gr.File(label=i18n('F0曲线文件, 可选, 一行一个音高, 代替默认F0及升降调'),visible=_B);file_index1=gr.Textbox(label=i18n(_y),placeholder='C:\\Users\\Desktop\\model_example.index',interactive=_A,visible=_B)
			...
			with gr.TabItem(i18n('批量推理')):
				...
				with gr.Row():
					with gr.Column():vc_transform1=gr.Number(label=i18n(_q),value=0);opt_input=gr.Textbox(label=i18n('指定输出文件夹'),value=_f);file_index3=gr.Textbox(label=i18n(_y),value='',interactive=_A);file_index4=gr.Dropdown(label=i18n(_r),choices=sorted(index_paths),interactive=_A);f0method1=gr.Radio(label=i18n(_s),choices=[_S,_T,'crepe',_N]if config.dml==_B else[_S,_T,_N],value=_N,interactive=_A);format1=gr.Radio(label=i18n(_z),choices=[_U,_V,_Z,_a],value=_U,interactive=_A);refresh_button.click(fn=lambda:change_choices()[1],inputs=[],outputs=file_index4,api_name='infer_refresh_batch')
					with gr.Column():resample_sr1=gr.Slider(minimum=0,maximum=48000,label=i18n(_t),value=0,step=1,interactive=_A);rms_mix_rate1=gr.Slider(minimum=0,maximum=1,label=i18n(_u),value=1,interactive=_A);protect1=gr.Slider(minimum=0,maximum=.5,label=i18n(_v),value=.33,step=.01,interactive=_A);filter_radius1=gr.Slider(minimum=0,maximum=7,label=i18n(_w),value=3,step=1,interactive=_A);index_rate2=gr.Slider(minimum=0,maximum=1,label=i18n(_x),value=1,interactive=_A)
				with gr.Row():dir_input=gr.Textbox(label=i18n('输入待处理音频文件夹路径(去文件管理器地址栏拷就行了)'),placeholder='C:\\Users\\Desktop\\input_vocal_dir');inputs=gr.File(file_count='multiple',label=i18n(_A0))
		...
		with gr.TabItem(i18n('伴奏人声分离&去混响&去回声')):
			with gr.Group():
				...
				with gr.Row():
					with gr.Column():dir_wav_input=gr.Textbox(label=i18n('输入待处理音频文件夹路径'),placeholder='C:\\Users\\Desktop\\todo-songs');wav_inputs=gr.File(file_count='multiple',label=i18n(_A0))
					with gr.Column():model_choose=gr.Dropdown(label=i18n('模型'),choices=uvr5_names);agg=gr.Slider(minimum=0,maximum=20,step=1,label='人声提取激进程度',value=10,interactive=_A,visible=_B);opt_vocal_root=gr.Textbox(label=i18n('指定输出主人声文件夹'),value=_f);opt_ins_root=gr.Textbox(label=i18n('指定输出非主人声文件夹'),value=_f);format0=gr.Radio(label=i18n(_z),choices=[_U,_V,_Z,_a],value=_V,interactive=_A)
		...
		with gr.TabItem(i18n('训练')):
			...
			with gr.Accordion(i18n('step3: 填写训练设置, 开始训练模型和索引')):
				...
				with gr.Row():file_dict={f:os.path.join(_A2,f)for f in os.listdir(_A2)};file_dict={k:v for(k,v)in file_dict.items()if k.endswith(_Q)};file_dict_g={k:v for(k,v)in file_dict.items()if'G'in k and _R in k};file_dict_d={k:v for(k,v)in file_dict.items()if'D'in k and _R in k};pretrained_G14=gr.Dropdown(label=i18n('加载预训练底模G路径'),choices=list(file_dict_g.values()),value=file_dict_g['f0G32k.pth'],interactive=_A);pretrained_D15=gr.Dropdown(label=i18n('加载预训练底模D路径'),choices=list(file_dict_d.values()),value=file_dict_d['f0D32k.pth'],interactive=_A);sr2.change(change_sr2,[sr2,if_f0_3,version19],[pretrained_G14,pretrained_D15]);version19.change(change_version19,[sr2,if_f0_3,version19],[pretrained_G14,pretrained_D15,sr2]);if_f0_3.change(change_f0,[if_f0_3,sr2,version19],[f0method8,gpus_rmvpe,pretrained_G14,pretrained_D15]);gpus16=gr.Textbox(label=i18n(_A1),value=gpus,interactive=_A);but3=gr.Button(i18n('训练模型'),variant=_C);but4=gr.Button(i18n('训练特征索引'),variant=_C);info3=gr.Textbox(label=i18n(_J),value='',max_lines=10);but3.click(click_train,[exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19],info3,api_name='train_start');but4.click(train_index,[exp_dir1,version19],info3)
		...
		with gr.TabItem(i18n('ckpt处理')):
			with gr.Group():
				...
				with gr.Row():sr_=gr.Radio(label=i18n(_g),choices=[_I,_O],value=_I,interactive=_A);if_f0_=gr.Radio(label=i18n('模型是否带音高指导'),choices=[i18n(_H),i18n('否')],value=i18n(_H),interactive=_A);info__=gr.Textbox(label=i18n('要置入的模型信息'),value='',max_lines=8,interactive=_A);name_to_save0=gr.Textbox(label=i18n('保存的模型名不带后缀'),value='',max_lines=1,interactive=_A);version_2=gr.Radio(label=i18n('模型版本型号'),choices=[_D,_M],value=_D,interactive=_A)
			...
			with gr.Group():
				...
				with gr.Row():ckpt_path0=gr.Textbox(label=i18n(_h),value='',interactive=_A);info_=gr.Textbox(label=i18n('要改的模型信息'),value='',max_lines=8,interactive=_A);name_to_save1=gr.Textbox(label=i18n('保存的文件名, 默认空为和源文件同名'),value='',max_lines=8,interactive=_A)
			...
			with gr.Group():
				...
				with gr.Row():ckpt_path1=gr.Textbox(label=i18n(_h),value='',interactive=_A);but8=gr.Button(i18n('查看'),variant=_C);info6=gr.Textbox(label=i18n(_J),value='',max_lines=8)
				but8.click(show_info,[ckpt_path1],info6,api_name='ckpt_show')
			with gr.Group():
				gr.Markdown(value=i18n('模型提取(输入logs文件夹下大文件模型路径),适用于训一半不想训了模型没有自动提取保存小文件模型,或者想测试中间模型的情况'))
				with gr.Row():ckpt_path2=gr.Textbox(label=i18n(_h),value='E:\\codes\\py39\\logs\\mi-test_f0_48k\\G_23333.pth',interactive=_A);save_name=gr.Textbox(label=i18n('保存名'),value='',interactive=_A);sr__=gr.Radio(label=i18n(_g),choices=[_W,_I,_O],value=_I,interactive=_A);if_f0__=gr.Radio(label=i18n('模型是否带音高指导,1是0否'),choices=['1','0'],value='1',interactive=_A);version_1=gr.Radio(label=i18n('模型版本型号'),choices=[_M],value=_M,interactive=_A);info___=gr.Textbox(label=i18n('要置入的模型信息'),value='',max_lines=8,interactive=_A);but9=gr.Button(i18n('提取'),variant=_C);info7=gr.Textbox(label=i18n(_J),value='',max_lines=8);ckpt_path2.change(change_info_,[ckpt_path2],[sr__,if_f0__,version_1])
