_A1='/kaggle/input/ax-rmd/pretrained_v2'
_A0='以-分隔输入使用的卡号, 例如   0-1-2   使用卡0和卡1和卡2'
_z='也可批量输入音频文件, 二选一, 优先读文件夹'
_y='multiple'
_x='导出文件格式'
_w='检索特征占比'
_v='>=3则使用对harvest音高识别的结果使用中值滤波，数值为滤波半径，使用可以削弱哑音'
_u='保护清辅音和呼吸声，防止电音撕裂等artifact，拉满0.5不开启，调低加大保护力度但可能降低索引效果'
...
import warnings,traceback,threading,shutil,logging,datetime
...
for name in os.listdir(weight_root):
	if name.endswith(_P):names.append(name)
index_paths=[]
...
def extract_f0_feature(gpus,n_p,f0method,if_f0,exp_dir,version19,gpus_rmvpe):
	...
	if if_f0:
		if f0method!=_V:cmd='"%s" infer/modules/train/extract/extract_f0_print.py "%s/logs/%s" %s %s'%(config.python_cmd,now_dir,exp_dir,n_p,f0method);logger.info(_O+cmd);p=Popen(cmd,shell=_A,cwd=now_dir);done=[_B];threading.Thread(target=if_done,args=(done,p)).start()
		elif gpus_rmvpe!='-':
...
def change_sr2(sr2,if_f0_3,version19):path_str=''if version19==_F else _Z;f0_str=_Q if if_f0_3 else'';return get_pretrained_models(path_str,f0_str,sr2)
def change_version19(sr2,if_f0_3,version19):
	path_str=''if version19==_F else _Z
	if sr2==_U and version19==_F:sr2=_I
	to_return_sr2={_T:[_I,_N],_C:_D,_X:sr2}if version19==_F else{_T:[_I,_N,_U],_C:_D,_X:sr2};f0_str=_Q if if_f0_3 else'';return*get_pretrained_models(path_str,f0_str,sr2),to_return_sr2
def change_f0(if_f0_3,sr2,version19):path_str=''if version19==_F else _Z;return{_a:if_f0_3,_C:_D},{_a:if_f0_3,_C:_D},*get_pretrained_models(path_str,_Q if if_f0_3==_A else'',sr2)
def click_train(exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19):
	...
	if version19==_F or sr2==_I:config_path='v1/%s.json'%sr2
	...
	logger.info(_O+cmd);p=Popen(cmd,shell=_A,cwd=now_dir);p.wait();return _k
...
def change_f0_method(f0method8):
	...
	return{_a:visible,_C:_D}
def save_to_wav(record_button):
	if record_button is None:0
	else:path_to_file=record_button;new_name=datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')+'.wav';target_path=os.path.join(_l,_m,os.path.basename(new_name));shutil.move(path_to_file,target_path);return target_path
def save_to_wav2(dropbox):
	file_path=dropbox.name;target_path=os.path.join(_l,_m,os.path.basename(file_path))
	...
	with gr.Tabs():
		with gr.TabItem(i18n('模型推理')):
			...
			with gr.TabItem(i18n('单次推理')):
				with gr.Group():
					with gr.Row():
						with gr.Column():dropbox=gr.Audio(sources='upload',label=i18n('Drag your audio here:'),type=_n);record_button=gr.Audio(sources='microphone',label=i18n('Or record an audio:'),type=_n)
						with gr.Column():vc_transform0=gr.Number(label=i18n(_o),value=0);input_audio0=gr.Textbox(label=i18n('输入待处理音频文件路径(默认是正确格式示例)'),placeholder='C:\\Users\\Desktop\\audio_example.wav');file_index1=gr.Textbox(label=i18n(_p),placeholder='C:\\Users\\Desktop\\model_example.index',interactive=_A);file_index2=gr.Dropdown(label=i18n(_q),choices=sorted(index_paths),interactive=_A);f0method0=gr.Radio(label=i18n(_r),choices=[_R,_S,'crepe',_M]if config.dml==_B else[_R,_S,_M],value=_M,interactive=_A)
			...
			with gr.TabItem(i18n('批量推理')):
				...
				with gr.Row():
					with gr.Column():vc_transform1=gr.Number(label=i18n(_o),value=0);opt_input=gr.Textbox(label=i18n('指定输出文件夹'),value=_b);file_index3=gr.Textbox(label=i18n(_p),value='',interactive=_A);file_index4=gr.Dropdown(label=i18n(_q),choices=sorted(index_paths),interactive=_A);f0method1=gr.Radio(label=i18n(_r),choices=[_R,_S,'crepe',_M]if config.dml==_B else[_R,_S,_M],value=_M,interactive=_A);format1=gr.Radio(label=i18n(_x),choices=[_c,_d,'mp3','m4a'],value=_c,interactive=_A);refresh_button.click(fn=lambda:change_choices()[1],inputs=[],outputs=file_index4,api_name='infer_refresh_batch')
		...
		with gr.TabItem(i18n('训练')):
			...
			with gr.Accordion(i18n('step2b: 使用CPU提取音高(如果模型带音高), 使用GPU提取特征(选择卡号)')):
				with gr.Row():
					with gr.Column():gpus6=gr.Textbox(label=i18n(_A0),value=gpus,interactive=_A,visible=F0GPUVisible);gpu_info9=gr.Textbox(label=i18n('显卡信息'),value=gpu_info,visible=F0GPUVisible)
					with gr.Column():f0method8=gr.Radio(label=i18n('选择音高提取算法:输入歌声可用pm提速,高质量语音但CPU差可用dio提速,harvest质量更好但慢,rmvpe效果最好且微吃CPU/GPU'),choices=[_R,_S,'dio',_M,_V],value=_V,interactive=_A);gpus_rmvpe=gr.Textbox(label=i18n('rmvpe卡号配置：以-分隔输入使用的不同进程卡号,例如0-0-1使用在卡0上跑2个进程并在卡1上跑1个进程'),value='%s-%s'%(gpus,gpus),interactive=_A,visible=F0GPUVisible)
			...
			with gr.Accordion(i18n('step3: 填写训练设置, 开始训练模型和索引')):
				...
				with gr.Row():file_dict={f:os.path.join(_A1,f)for f in os.listdir(_A1)};file_dict={k:v for(k,v)in file_dict.items()if k.endswith(_P)};file_dict_g={k:v for(k,v)in file_dict.items()if'G'in k and _Q in k};file_dict_d={k:v for(k,v)in file_dict.items()if'D'in k and _Q in k};pretrained_G14=gr.Dropdown(label=i18n('加载预训练底模G路径'),choices=list(file_dict_g.values()),value=file_dict_g['f0G32k.pth'],interactive=_A);pretrained_D15=gr.Dropdown(label=i18n('加载预训练底模D路径'),choices=list(file_dict_d.values()),value=file_dict_d['f0D32k.pth'],interactive=_A);sr2.change(change_sr2,[sr2,if_f0_3,version19],[pretrained_G14,pretrained_D15]);version19.change(change_version19,[sr2,if_f0_3,version19],[pretrained_G14,pretrained_D15,sr2]);if_f0_3.change(change_f0,[if_f0_3,sr2,version19],[f0method8,gpus_rmvpe,pretrained_G14,pretrained_D15]);gpus16=gr.Textbox(label=i18n(_A0),value=gpus,interactive=_A);but3=gr.Button(i18n('训练模型'),variant=_E);but4=gr.Button(i18n('训练特征索引'),variant=_E);info3=gr.Textbox(label=i18n(_J),value='',max_lines=10);but3.click(click_train,[exp_dir1,sr2,if_f0_3,spk_id5,save_epoch10,total_epoch11,batch_size12,if_save_latest13,pretrained_G14,pretrained_D15,gpus16,if_cache_gpu17,if_save_every_weights18,version19],info3,api_name='train_start');but4.click(train_index,[exp_dir1,version19],info3)
		with gr.TabItem(i18n('ckpt处理')):
			...
			with gr.Group():
				...
				with gr.Row():ckpt_path1=gr.Textbox(label=i18n(_f),value='',interactive=_A);but8=gr.Button(i18n('查看'),variant=_E);info6=gr.Textbox(label=i18n(_J),value='',max_lines=8)
				but8.click(show_info,[ckpt_path1],info6,api_name='ckpt_show')
			with gr.Group():
				gr.Markdown(value=i18n('模型提取(输入logs文件夹下大文件模型路径),适用于训一半不想训了模型没有自动提取保存小文件模型,或者想测试中间模型的情况'))
				with gr.Row():ckpt_path2=gr.Textbox(label=i18n(_f),value='E:\\codes\\py39\\logs\\mi-test_f0_48k\\G_23333.pth',interactive=_A);save_name=gr.Textbox(label=i18n('保存名'),value='',interactive=_A);sr__=gr.Radio(label=i18n(_e),choices=[_U,_I,_N],value=_I,interactive=_A);if_f0__=gr.Radio(label=i18n('模型是否带音高指导,1是0否'),choices=['1','0'],value='1',interactive=_A);version_1=gr.Radio(label=i18n('模型版本型号'),choices=[_L],value=_L,interactive=_A);info___=gr.Textbox(label=i18n('要置入的模型信息'),value='',max_lines=8,interactive=_A);but9=gr.Button(i18n('提取'),variant=_E);info7=gr.Textbox(label=i18n(_J),value='',max_lines=8);ckpt_path2.change(change_info_,[ckpt_path2],[sr__,if_f0__,version_1])
