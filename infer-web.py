...
with gr.Blocks(theme="JohnSmith9982/small_and_pretty", title="AX RVC 🍏") as app:
    ...
    with gr.Tabs():
        with gr.TabItem(i18n("模型推理")):
            ...
            with gr.Group():
                gr.Markdown(
                    i18n("男转女推荐+12key, 女转男推荐-12key, 如果音域爆炸导致音色失真也可以自己调整到合适音域. ")
                )
                with gr.Row():
                    with gr.Column():
            ...
            with gr.Group():
                gr.Markdown(
                    i18n("批量转换, 输入待转换音频文件夹, 或上传多个音频文件, 在指定文件夹(默认opt)下输出转换的音频. ")
                )
                with gr.Row():
                    with gr.Column():
        ...
        with gr.TabItem(i18n("训练")):
            ...
            with gr.Group():
                gr.Markdown(i18n("step3: 填写训练设置, 开始训练模型和索引"))
                with gr.Row():
        ...
        with gr.TabItem(i18n("ckpt处理")):
            with gr.Group():
                gr.Markdown(i18n("模型融合, 可用于测试音色融合"))
                with gr.Row():
                    ckpt_a = gr.Textbox(
                        label=i18n("A模型路径"),
                        value="",
            ...
            with gr.Group():
                gr.Markdown(i18n("修改模型信息(仅支持weights文件夹下提取的小模型文件)"))
                with gr.Row():  ######
            ...
            with gr.Group():
                gr.Markdown(i18n("查看模型信息(仅支持weights文件夹下提取的小模型文件)"))
                with gr.Row():
